<template>
  <div style="position: relative">
    <div class="action-group padding-xs" style="position: absolute; right: 0px; top: 0px;z-index:99">
      <div v-if="selectedNode" class="action-item tsfont-arrow-up" @click="toFront()">移到最顶端</div>
      <div v-if="selectedNode" class="action-item tsfont-arrow-down" @click="toBack()">移到最底端</div>
    </div>
...
import { WidgetFactory } from '@/views/pages/diagram/graph/core/WidgetFactory.js';
      ...
      if (!this.graph) {
        let graphConfig = {
          ...
          interacting: cellView => {
            return { nodeMovable: !cellView.cell.getData()['disableDrag'] };
          },
          connecting: {
            // 配置全局的连线规则
        ...
        this.graph.use(
          new Transform({
            resizing: {
              enabled: node => {
                return node.getProp('setting') && !!node.getProp('setting')['resizable'];
              }
            }
          })
        );
