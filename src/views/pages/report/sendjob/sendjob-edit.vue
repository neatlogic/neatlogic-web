  ...
  <div class="sendjob-edit">
    <TsContain border="border">
      <template v-slot:navigation>
        <span class="tsfont-left text-action" @click="$back('/sendjob-manage')">{{ $t('common.back') }}</span>
      </template>
      ...
      <template v-slot:topRight>
        <div class="action-group">
          ...
          <div class="action-item  text-action tsfont-eye" @click="previewJob">{{ $t('common.preview') }}</div>
                ...
                <li v-for="(report, index) in jobConfig.reportList" :key="report.id" class="report-item">
                  <div class="flex-between">
                    ...
                    <ul class="action-group">
                      <li class="action-item last text-action tsfont-eye" @click="previewReport(report)">{{ $t('common.preview') }}</li>
  ...
  methods: {
    ...
    async saveJob() {
      ...
      if (this.jobConfig.reportList.length === 0) {
        this.$Notice.warning({ title: this.$t('common.error'), desc: this.$t('term.report.describe.reportIsEmpty') });
        return false;
      }
