<template>
  <div>
    ...
    <div class="ivu-form-item tsform-item ivu-form-label-top">
      <label class="ivu-form-item-label overflow">显示面积</label>
      <div class="ivu-form-item-content">
        <TsFormSwitch
          :value="config.area ? true : false"
          :true-value="true"
          :false-value="false"
          @change="
            val => {
              if (val) {
                setConfigValue('area', { smooth: true });
              } else {
                setConfigValue('area', null);
              }
            }
          "
        ></TsFormSwitch>
...
export default {
  ...
  data() {
    return {
      layoutList: [
        { value: 'horizontal', text: '横向布局' },
        { value: 'vertical', text: '纵向布局' }
      ],
      positionList: [
