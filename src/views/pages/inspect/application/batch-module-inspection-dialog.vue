    ...
    >
      <ul>
        <li v-for="(item, index) in noCompobIdList" :key="index" class="flex-start mb-sm">
          ...
          未配置
          <span v-if="$AuthUtils.hasRole('INSPECT_MODIFY')" class="text-href" @click="openInspectionTool(item.toolName)">巡检工具</span>
      ...
      this.$api.inspect.applicationInspect
        ...
        .then(res => {
          if (res && res.Status == 'OK') {
            ...
            this.envList &&
              this.envList.forEach(item => {
                item.dataList = [{ text: item.env && item.env.name ? `${item.env.name}环境` : '', value: item.env && item.env.id ? item.env.id : null }];
                item.value = [];
              });
          }
        })
    ...
    isValid() {
      ...
      if (copyEnvList && !this.$utils.isEmpty(copyEnvList)) {
        ...
        if (this.$utils.isEmpty(emptyList)) {
          this.$Message.warning('请至少选择一个环境');
          isValid = false;
        }
      }
      return isValid;
