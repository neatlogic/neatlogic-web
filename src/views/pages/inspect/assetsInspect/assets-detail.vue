  ...
  <div>
    <TsContain siderPosition="right" :isSiderHide="isHistoryHide" :siderWidth="180">
      <template v-slot:navigation>
        <span class="text-action ts-angle-left" @click="$back('/assets-inspection-manage')">{{ $getFromPage('返回') }}</span>
      </template>
      ...
      <template v-slot:topRight>
        <div class="bar-top-right action-group text-right">
          <span class="action-item tsfont-history" @click="isHistoryHide = !isHistoryHide">历史报告</span>
          <span v-if="isDownloading" class="action-item tsfont-download disable" title="正在导出数据，请耐心等候">
            导出
