            ...
            <template v-slot:action="{ row }">
              <div class="tstable-action">
                <ul class="tstable-action-ul">
                  <li v-auth="'INSPECT_CONFIG_FILE_MODIFY'" class="tsfont-file-single" @click="clearFile(row)">清理文件</li>
                  <li v-if="row.versionCount && row.versionCount > 1" class="tsfont-compare" @click="compareFile(row)">对比</li>
                </ul>
              </div>
            </template>
          </TsTable>
