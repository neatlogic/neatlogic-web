  ...
  <div>
    <TsContain siderPosition="right" :siderWidth="200" :isSiderHide="isSiderHideHistory">
      <template v-slot:navigation>
        <span class="tsfont-left text-action" @click="$back('/configfile-manage')">{{ $getFromPage('返回') }}</span>
      </template>
      ...
      <template v-slot:topRight>
        <div class="action-group">
          <span v-if="versionDataConfig.tbodyList && versionDataConfig.tbodyList.length > 1" class="action-item  tsfont-compare" @click="compareFile">对比</span>
          <span class="action-item tsfont-history" @click="openHistory">历史版本</span>
        </div>
      </template>
            ...
            <TabPane label="变更记录" name="changeRecord">
              <div class="list-content">
                <template v-if="versionDataConfig.tbodyList && versionDataConfig.tbodyList.length > 0">
                  ...
                  <div v-if="versionDataConfig.currentPage < versionDataConfig.pageCount" class="text-href pt-sm pl-nm" @click="nextPage('version')">查看更多</div>
                </template>
                <NoData v-else></NoData>
              </div>
            </TabPane>
            <TabPane label="扫描历史" name="scanHistory">
              <div class="list-content pl-nm">
                <template v-if="historyDataConfig.tbodyList && historyDataConfig.tbodyList.length > 0">
                  ...
                  <div v-if="historyDataConfig.currentPage < historyDataConfig.pageCount" class="text-href" @click="nextPage('history')">查看更多</div>
                </template>
                <NoData v-else></NoData>
              </div>
            </TabPane>
