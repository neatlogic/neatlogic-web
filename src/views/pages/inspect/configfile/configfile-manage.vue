            ...
            <template v-slot:action="{ row }">
              <div class="tstable-action">
                <ul class="tstable-action-ul">
                  <li class="tsfont-file-single" @click="openFileList(row)">文件列表</li>
                  <li v-auth="'INSPECT_CONFIG_FILE_MODIFY'" class="tsfont-setting" @click="editFilePath(row)">路径配置</li>
                  <li v-auth="'INSPECT_CONFIG_FILE_MODIFY'" class="tsfont-trash-o" @click="clearFile(row)">清理文件</li>
                </ul>
              </div>
            </template>
          </TsTable>
...
export default {
  ...
  methods: {
    ...
    openEditDialog(type) {
      ...
      if (type == 'add') {
        this.editTitle = '批量添加路径';
      } else if (type == 'delete') {
        this.editTitle = '批量删除路径';
      } else if (type == 'clear') {
        this.editTitle = '批量清理文件';
      }
    ...
    clearFile(row) {
      this.$createDialog({
        title: '清理文件',
        content: '清理' + row.ip + (row.name ? '[' + row.name + ']' : '') + '所有配置文件记录',
        btnType: 'error',
        okText: '清理',
