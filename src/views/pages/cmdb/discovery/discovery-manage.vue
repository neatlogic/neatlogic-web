    ...
    <TsContain :enableCollapse="true">
      <div slot="topLeft">
        <div class="action-group">
          <div class="action-item tsfont-setting" @click="toDiscoveryRule">发现规则</div>
            ...
            <div class="mt-nm">
              ...
              >
                ...
                <template v-slot:control="{ row }">
                  <div v-if="row.dataCount > 0" class="tsfont-search" @click="toDiscoveryData(row.name)">数据</div>
                  <div class="tsfont-edit" @click="toSyncManager(row.type, row.name)">配置</div>
                </template>
              </TsCard>
            </div>
          </div>
            ...
            <template v-slot:action="{ row }">
              <div class="tstable-action">
                <ul class="tstable-action-ul">
                  <li v-if="row.dataCount > 0" class="tsfont-search" @click="toDiscoveryData(row.name)">数据</li>
                  <li class="tsfont-edit" @click="toSyncManager(row.type, row.name)">配置</li>
                </ul>
              </div>
            </template>
          </TsTable>
...
export default {
  ...
  data() {
    return {
      showMode: 'card',
      theadList: [{ key: 'name', title: '名称' }, { key: 'type', title: '分类' }, { key: 'dataCount', title: '数据量' }, { key: 'action' }],
