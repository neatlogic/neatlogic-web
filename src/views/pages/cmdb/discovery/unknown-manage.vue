      ...
      <template v-slot:content>
        ...
        >
          <template v-slot:rule="{ row }">
            <span v-if="row.rule" class="text-info">已就绪</span>
            <span v-else class="text-grey">未定义</span>
          </template>
          <template
          ...
          <template v-slot:action="{ row }">
            <div class="tstable-action">
              <ul class="tstable-action-ul">
                <li v-if="!row.rule" class="tsfont-edit" @click="editRule({ sysObjectId: row['SYS_OBJECT_ID'] })">添加规则</li>
                <li v-else class="tsfont-edit" @click="editRule(row.rule)">编辑规则</li>
  ...
  methods: {
    ...
    searchCollectionData(currentPage) {
      ...
      this.$api.cmdb.sync.searchCollectionData(this.searchParam).then(res => {
        ...
        this.collectionData.theadList.unshift({ key: 'rule', title: '匹配规则' });
