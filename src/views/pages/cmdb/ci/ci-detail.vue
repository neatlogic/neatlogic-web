  ...
  methods: {
    ...
    deleteCi: function() {
      ...
      this.$createDialog({
        ...
        'on-ok': function(vnode) {
          _this.$api.cmdb.ci.deleteCiById(_this.ciId).then(res => {
            if (res.Status == 'OK') {
              _this.$Message.success(this.$t('message.content.deletesuccess'));
              _this.$router.push({ path: '/ci-manage' });
              _this.getMenuList();
              vnode.isShow = false;
            }
          });
    ...
    deleteAttr: function(attr) {
      ...
      this.$createDialog({
        ...
        btnType: 'error',
        'on-ok': function(vnode) {
          _this.$api.cmdb.ci.deleteAttrById(attr.id).then(res => {
            _this.$Message.success(this.$t('message.content.deletesuccess'));
            _this.getAttrByCiId();
            vnode.isShow = false;
          });
    ...
    deleteRel: function(rel) {
      ...
      this.$createDialog({
        ...
        btnType: 'error',
        'on-ok': function(vnode) {
          _this.$api.cmdb.ci.deleteRelById(rel.id).then(res => {
            _this.$Message.success(this.$t('message.content.deletesuccess'));
            _this.getRelByCiId();
            vnode.isShow = false;
          });
