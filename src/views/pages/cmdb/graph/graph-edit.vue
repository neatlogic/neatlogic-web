      ...
      <template v-slot:topRight>
        <div class="action-group">
          ...
          <span class="action-item">
            <Button type="primary" @click="saveGraph(true)">{{ $t('button.save') }}</Button>
          </span>
          <span v-if="graphData.id && invokeGraphList.length === 0" class="action-item">
            <Button type="error" @click="deleteGraph()">{{ $t('button.delete') }}</Button>
          </span>
        </div>
      </template>
      ...
      <template v-slot:right>
        <div class="margin-md">
          ...
          <div v-else>
            ...
            <TsFormItem v-if="graphData.type === 'public'" :label="$t('form.label.auth')" labelPosition="top">
              <UserSelect
            ...
            <TsFormItem :label="$t('form.label.backgroundcolor')" labelPosition="top">
              <div class="padding-sm radius-sm">
                <ColorPicker
                  :transfer="true"
                  recommend
            ...
            <TsFormItem :label="$t('form.label.backgroundimage')" labelPosition="top">
              <div class="padding-sm radius-sm">
            ...
            <TsFormItem :label="$t('form.label.description')" labelPosition="top">
              <div class="padding-sm radius-sm">
                <TsFormInput
  ...
  methods: {
    ...
    searchGraph(currentPage) {
      ...
      this.$api.cmdb.graph.searchGraph(this.searchGraphParam).then(res => {
        ...
        //添加新视图入口
        this.graphSearchData.tbodyList.unshift({ icon: 'tsfont-plus', name: this.$t('term.cmdb.newgraph') });
      });
    },
    doSearch(keyword) {
