...
import { ElementFactory } from '@/views/pages/cmdb/graph/graph-editor/element/core/ElementFactory.js';
export default {
  name: '',
  components: {
  ...
  methods: {
    ...
    init: function() {
      if (!this.graph) {
        ...
        this.graph.on('node:embed', ({ node }) => {
        });
        this.graph.on('node:embedding', ({ e, node }) => {
