      ...
      <template v-slot:topLeft>
        <div class="action-group">
          <span v-if="resourceIdList.length == 0" class="action-item disable">
            ...
            ><span>批量操作</span><span class="tsfont-down"></span>
            </div>
          </span>
          <span v-else class="action-item">
            <Dropdown v-auth="['RESOURCECENTER_MODIFY']" trigger="click" placement="bottom-start">
              ...
              <DropdownMenu slot="list">
                <DropdownItem>
                  <div class="more-action" @click="addTag">添加标签</div>
                </DropdownItem>
                <DropdownItem>
                  <div class="more-action" @click="delTag">删除标签</div>
                </DropdownItem>
                <DropdownItem>
                  <div class="more-action" @click="addAccount">添加帐号</div>
                </DropdownItem>
                <DropdownItem>
                  <div class="more-action" @click="delAccount">删除帐号</div>
                </DropdownItem>
                <DropdownItem>
                  <div class="more-action" @click="delSelectedAsset">删除资产</div>
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
          </span>
          <span v-if="ciData && ciData.isAbstract === 0 && ciData.isVirtual === 0" v-auth="['RESOURCECENTER_MODIFY']" class="action-item">
            <Button type="primary" @click="addAsset">添加资产</Button>
          </span>
          <span class="action-item tsfont-export" @click="openExportDialog">导出</span>
        </div>
      </template>
          ...
          <template v-slot:monitorTime="{ row }">
            ...
            <span v-if="row.monitorStatusJson && row.monitorTime" :title="row.monitorTime | formatDate">
              <span class="inspectTime-class">
                ...
                <span class="text-title">{{ formatTime(row.monitorTime) | formatTimeCost({ unitNumber: 1, language: 'zh', unit: 'minute' }) }} 之前</span>
              </span>
            </span>
          ...
          <template v-slot:inspectTime="{ row }">
            ...
            <span v-if="row.inspectStatusJson && row.inspectTime" :title="row.inspectTime | formatDate">
              <span class="inspectTime-class">
                ...
                <span class="text-title">{{ formatTime(row.inspectTime) | formatTimeCost({ unitNumber: 1, language: 'zh', unit: 'minute' }) }} 之前</span>
              </span>
          ...
          <template v-slot:action="{ row }">
            ...
            <div v-auth="'RESOURCECENTER_MODIFY'" class="tstable-action">
              <ul class="tstable-action-ul">
                <li class="ts-tag" @click="tagEdit(row)">标签管理</li>
                <li class="ts-user" @click="editAccount(row)">帐号管理</li>
                <li class="tsfont-edit" @click="editAsset(row)">编辑资产</li>
                <li class="tsfont-trash-o" @click="deleteAsset(row)">删除资产</li>
              </ul>
            </div>
          </template>
        </TsTable>
...
export default {
  ...
  methods: {
      ...
      return this.$api.cmdb.asset.getResourceTreeType().then(res => {
        ...
        if (this.selectType.typeId) {
          ...
          if (data[0]) {
            ...
            this.implementName = data[0].label + '(' + data[0].name + ') 巡检';
          }
        }
        this.treeData = data;
      });
    ...
    selectTreeNode(nodeList, node) {
      ...
      this.implementName = node.label + '(' + node.name + ') 巡检';
    ...
    // 批量操作
    addTag() {
      this.batchAction('批量添加标签', 'addTag');
    },
    addAccount() {
      this.accountAction('批量添加帐号', 'addAccount');
    },
    delTag() {
      this.batchAction('批量删除标签', 'delTag');
    },
    delAccount() {
      this.accountAction('批量删除帐号', 'delAccount');
    },
    // 刷新标签列表
    refreshTagList() {
      this.listForselect();
