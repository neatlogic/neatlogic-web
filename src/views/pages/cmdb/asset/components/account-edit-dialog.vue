  ...
  <div>
    <TsDialog v-bind="dialogConfig" @on-close="closeDialog">
      ...
      >
        ...
        <template v-slot:privateAccount>
          <div>
            ...
            <div class="text-href tsfont-plus" @click="openPrivateAccount">私有帐号</div>
      ...
      <template v-slot:footer>
        ...
        <Button type="primary" @click="testConnect">测试连接</Button>
  ...
  methods: {
    ...
    validRequired() {
      ...
      if (accountIdList && accountIdList.length == 0) {
        this.$Notice.error({ title: '温馨提示', desc: '公共帐号和私有帐号的集合不能为空，请至少填写一个帐号' });
    ...
    saveAccount() {
      ...
      this.$api.cmdb.asset.saveSingleAccount(data).then(res => {
        if (res.Status == 'OK') {
          ...
          this.handleTipsMessage(res, this.$t('message.content.savesuccess'));
        }
      });
    },
    getAccountIdList() {
