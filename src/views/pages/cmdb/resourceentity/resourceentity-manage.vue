    ...
    <TsContain>
      <template v-slot:topLeft>
        <div class="action-group">
          <span class="action-item tsfont-rotate-right" @click="rebuildAll()">重建所有视图</span>
        </div>
      </template>
      ...
      <template v-slot:content>
        <TsTable v-if="tbodyList" :theadList="theadList" :tbodyList="finalTbodyList">
          ...
          <template v-slot:type="{ row }">
            <span v-if="row.type === 'resource'">资源对象</span>
            <span v-else-if="row.type === 'scene'">场景视图</span>
          </template>
          <template v-slot:status="{ row }">
            <span v-if="row.status === 'error'" class="text-error">异常</span>
            <span v-if="row.error">
            ...
            <span v-else-if="row.status === 'ready'" class="text-success">就绪</span>
            <span v-else-if="row.status === 'pending'" class="text-grey">未创建</span>
          </template>
        </TsTable>
      </template>
    </TsContain>
...
export default {
  ...
  data() {
    return {
      ...
      isEditShow: false,
      theadList: [{ key: 'name', title: '视图' }, { key: 'label', title: '名称' }, { key: 'type', title: '类型' }, { key: 'status', title: '状态' }, { key: 'initTime', title: '初始化时间', type: 'time' }, { key: 'description', title: '说明' }, { key: 'action' }],
      tbodyList: []
    };
  },
  beforeCreate() {},
  ...
  computed: {
    finalTbodyList() {
      if (this.keyword) {
        ...
        return this.tbodyList.filter(d => (d.name && d.name.toLowerCase().includes(k)) ||
         (d.label && d.label.toLowerCase().includes(k)) || 
