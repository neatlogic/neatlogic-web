  ...
  <div>
    <div v-if="needCondition">
      <div class="clearfix" style="margin-bottom:10px">
        <div v-if="needAction && ciEntityData && ciEntityData.tbodyList && ciEntityData.tbodyList.length > 0" class="batch">
          <Dropdown trigger="click">
            <Button type="primary" ghost :disabled="!selectedCiEntityList || selectedCiEntityList.length == 0">
              {{ $t('button.batchaction') }}
              <span class="tsfont-down"></span>
            </Button>
            <DropdownMenu slot="list">
              <DropdownItem @click.native="batchEdit()">{{ $t('button.edit') }}</DropdownItem>
              <DropdownItem @click.native="batchDelete()">{{ $t('button.delete') }}</DropdownItem>
            </DropdownMenu>
          </Dropdown>
        </div>
        ...
        <div v-if="!isAdvancedSearch && needExport" class="export">
          <Button type="primary" :ghost="true" @click="isExportDialogShow = true">{{ $t('button.export') }}</Button>
        </div>
    ...
    <div v-if="ciEntityData && ciEntityData.tbodyList && ciEntityData.tbodyList.length > 0">
      ...
      >
        ...
        <template v-slot:expand="{ row }">
          ...
          <div v-else>{{ $t('message.content.nothingchange') }}</div>
        </template>
            ...
            <Dropdown v-if="needAction" :transfer="true">
              <a href="javascript:void(0)">
                <b :class="row.actionType == 'delete' ? 'text-error' : 'text-primary'">
                  {{ row.actionType == 'delete' ? $t('button.delete') : $t('button.edit') }}
              ...
              <DropdownMenu v-if="actionTypeConfig && (actionTypeConfig.edit || actionTypeConfig.del)" slot="list">
                <DropdownItem v-if="actionTypeConfig && actionTypeConfig.edit">
                  <a href="javascript:void(0)" @click="changeActionType(row, 'update')">{{ $t('button.edit') }}</a>
                </DropdownItem>
                <DropdownItem v-if="actionTypeConfig && actionTypeConfig.del">
                  <a href="javascript:void(0)" @click="changeActionType(row, 'delete')">{{ $t('button.delete') }}</a>
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
            <b v-if="!needAction" :class="row.actionType == 'delete' ? 'text-error' : 'text-primary'">{{ row.actionType == 'delete' ? $t('button.delete') : $t('button.edit') }}</b>
          </div>
        </template>
    ...
    >
      ...
      <template v-slot:footer>
        <Button @click="isExportDialogShow = false">{{ $t('button.cancel') }}</Button>
        <Button type="primary" ghost @click="selectAllExportItem()">{{ $t('button.selectall') }}</Button>
        <Button
...
export default {
  ...
  data() {
    return {
      ...
      sortConfig: {},
      actionTypeList: [
        { value: 'update', text: this.$t('button.edit') },
        { value: 'delete', text: this.$t('button.delete') }
    ...
    batchEdit() {
      if (this.selectedCiEntityList && this.selectedCiEntityList.length > 0) {
        if (this.saveMode == 'save') {
          ...
          if (!isSameCi) {
            this.$Message.info(this.$t('message.content.cmdb.diffci'));
            return;
          }
          if (errorList.length > 0) {
            this.$Message.info(this.$t('message.content.cmdb.hasnoauth', { target: errorList.map(cientity => cientity.name).join(',') }));
            return;
          }
