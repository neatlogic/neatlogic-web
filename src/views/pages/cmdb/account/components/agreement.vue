  ...
  >
    <template v-slot>
      <div class="topSearch pb-nm">
        <span v-auth="'RESOURCECENTER_ACCOUNT_MODIFY'" class="text-action tsfont-plus" @click="add()">协议</span>
        <TsFormInput
          v-model="keywords"
        ...
        <template v-slot:action="{ row }">
          <div v-auth="'RESOURCECENTER_ACCOUNT_MODIFY'" class="tstable-action">
            <ul class="tstable-action-ul">
              <li
                @click="edit(row)"
              ><span v-if="row.disabled" class="tsfont-edit">编辑</span><span v-else>保存</span></li>
              <li
                @click="del(row)"
              ><span v-if="row.disabled" class="tsfont-trash-o">删除</span><span v-else>取消</span></li>
            </ul>
          </div>
        </template>
      </TsTable>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      theadList: [
        { key: 'name', title: '名称', width: '250' },
        { key: 'port', title: '默认端口', width: '250' },
        { key: 'action', title: ''}
      ]
    };
  },
