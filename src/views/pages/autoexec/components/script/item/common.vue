...
<template>
  <div v-if="step" class="script-detail">
    ...
    <div v-else class="line-2 text-tip">
      {{ $t('page.notarget', {target: $t('term.autoexec.inputparam')}) }}
    </div>
    <!-- 自由参数 -->
    ...
    <!-- 输出参数 -->
    <div class="pt-nm pb-sm text-title tips"><span>
      {{ $t('term.autoexec.outputparameter') }}
    </span></div>
    ...
    <div v-else class="line-2 text-tip">
      {{ $t('page.notarget', {target: $t('term.autoexec.outputparameter')}) }}
    </div>
  </div>
</template>
<script>
