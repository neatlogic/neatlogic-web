      ...
      <li v-for="(step, sindex) in list" :id="'id_' + combopConfig.combopUuid + '_' + step.uuid" :key="sindex+'_'+step.uuid">
            ...
            >
              <div class="script-title border-color" @click="toggleshow(step)">
                <div class="overflow">
                  ...
                  <span class="text-action name" :title="step.operationName+(step.letter?('_'+step.letter):'')" @click.stop="gotoDateil(step)">{{ step.operationName }}{{ step.letter?('_'+step.letter):'' }}</span>
        ...
        <div v-else class="inner-block radius-lg hide-li" :class="{'bg-op': step.operationName != 'native/IF-Block', 'border-base': step.operationName == 'native/IF-Block'}">
          <div class="script-title border-color">
            <div class="overflow">
              【{{ typeText(step.operationType) }}】
              <span class="name" :title="step.operationName+(step.letter?('_'+step.letter):'')">{{ step.operationName }}{{ step.letter?('_'+step.letter):'' }}</span>
            </div>
  ...
  methods: {
    ...
    deleteScript(item, index) {
      this.$createDialog({
        ...
        content: this.$t('dialog.content.deleteconfirm', {target: item.operationName + (item.letter ? ('_' + item.letter) : '')}),
        btnType: 'error',
        'on-ok': vnode => {
          vnode.isShow = false;
          this.$nextTick(() => {
  ...
  computed: {
    ...
    typeText() {
      return type => {
        ...
        if (type == 'script') {
          text = this.$t('term.autoexec.customize');
        } else if (type == 'tool') {
