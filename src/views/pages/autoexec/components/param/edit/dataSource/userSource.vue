<template>
  ...
  >
    <template v-slot>
      ...
      <TsFormItem :label="$t('term.autoexec.singleselection') + '/' + $t('term.autoexec.multipleselection')" required>
        <TsFormRadio
          v-model="isMultiple"
          :dataList="dataList"
          :validateList="validateList"
...
export default {
  ...
  data() {
    return {
      dialogConfig: {
        title: this.$t('page.edittarget', {target: this.$t('term.autoexec.datasource')}),
        type: 'modal',
        isShow: true
      },
      ...
      dataList: [
        { text: this.$t('term.autoexec.singleselection'), value: 0 },
        { text: this.$t('term.autoexec.multipleselection'), value: 1 }
      ],
