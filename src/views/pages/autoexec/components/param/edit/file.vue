            ...
            <div class="search">
              <div style="width:240px">
                <Input
                  v-model="keyword"
                  search
                  :placeholder="$t('form.placeholder.pleaseselect', {target: ''})"
...
export default {
  ...
  props: {
    ...
    title: {
      ...
      type: String,
      default() {
        return this.$t('page.uploadattachment');
      }
    },
    multiple: {
      //是否支持多文件上传，默认否
  ...
  methods: {
    ...
    success(res, file, fileList) {
      if (res.Status == 'OK') {
        ...
        this.$Message.success(this.$t('message.content.uploadsuccess'));
    ...
    error(res, file, fileList) {
      if (file.Status == 'ERROR') {
        this.$Notice.error({
          title: this.$t('message.content.uploadfailed'),
          desc: file.Message
        });
      }
    },
  ...
  watch: {
    value: {
      handler(newVal) {
        ...
        } else if (this.$utils.isEmpty(newVal)) {
