      ...
      <div ref="tagDiv" class="pl-sm tag-item">
        <span v-for="(v, index) in searchConditionList" :key="index">
          ...
          <span v-for="(item, vIndex) in v.conditionList" :key="vIndex">
            ...
            <span v-if="index != (searchConditionList.length -1) && vIndex != v.conditionList.length - 1" class="text-href pr-sm">{{ item.childJoinType == 'or' ? $t('term.expression.or') : (item.childJoinType == 'and' ? $t('term.expression.and') : '') }}</span>
            <span v-else-if="vIndex != v.conditionList.length - 1" class="text-href pr-sm">{{ item.childJoinType == 'or' ? $t('term.expression.or') : (item.childJoinType == 'and' ? $t('term.expression.and') : '') }}</span>
          </span>
          <span v-if="searchConditionList.length > 1">)</span>
          <span v-if="index != (searchConditionList.length -1)" class="text-href pl-sm pr-sm">{{ v.joinType == 'or' ? $t('term.expression.or') : (v.joinType == 'and' ? $t('term.expression.and') : '') }}</span>
        </span>
      </div>
          ...
          <DropdownMenu slot="list">
            <div v-if="isVisible">
              <div class="dropdown-box pl-nm pr-nm">
                <span class="tsfont-plus text-action mb-nm" @click="addGroupCondition">{{ $t('term.autoexec.combinationcondition') }}</span>
                <div v-for="(v, tIndex) in conditionList" :key="v.uuid + tIndex">
                    ...
                    <TsRow v-for="(item, index) in v.conditionList" :key="index">
                      ...
                      <Col span="3">
                        <div class="action-group">
                          <span class="action-item tsfont-plus" :title="$t('dialog.title.addtarget', {target: $t('term.autoexec.combinationcondition')})" @click.stop="addChildCondition(tIndex)"></span>
                          <span
                            v-if="(tIndex == 0 && index == 0 ? false : true) && !disabledUuidList.includes(item.uuid)"
                            class="action-item tsfont-trash-s"
                            :title="$t('dialog.title.deletetarget', {target: $t('term.autoexec.combinationcondition')})"
                      ...
                      <div v-if="index != v.conditionList.length - 1" class="text-center">
                        ...
                        >
                          <a href="javascript:void(0)">
                            <span v-if="item.joinType == 'and'">{{ $t('term.expression.and') }}</span>
                            <span v-else-if="item.joinType == 'or'">{{ $t('term.expression.or') }}</span>
                            <Icon type="ios-arrow-down"></Icon>
                          </a>
                          <DropdownMenu slot="list">
                            <DropdownItem name="and" :disabled="(disabledUuidList.includes(item.uuid) && (v.conditionList[index + 1] && !disabledUuidList.includes(v.conditionList[index + 1]['uuid']))) ? false : disabledUuidList.includes(item.uuid)">{{ $t('term.expression.and') }}</DropdownItem>
                            <DropdownItem name="or" :disabled="(disabledUuidList.includes(item.uuid) && (v.conditionList[index + 1] && !disabledUuidList.includes(v.conditionList[index + 1]['uuid']))) ? false : disabledUuidList.includes(item.uuid)">{{ $t('term.expression.or') }}</DropdownItem>
                          </DropdownMenu>
                        </Dropdown>
                      </div>
                    </TsRow>
                  ...
                  <div v-if="tIndex != conditionList.length - 1" class="text-center">
                    ...
                    >hou
                      <a href="javascript:void(0)">
                        <span v-if="v.joinType == 'and'">{{ $t('term.expression.and') }}</span>
                        <span v-else-if="v.joinType == 'or'">{{ $t('term.expression.or') }}</span>
                        <Icon type="ios-arrow-down"></Icon>
                      </a>
                      <DropdownMenu slot="list">
                        <!-- 禁用后有条件，取消禁用 -->
                        <DropdownItem name="and" :disabled="(disabledGroupUuidList.includes(v.uuid) && (conditionList[tIndex + 1] && !disabledGroupUuidList.includes(conditionList[tIndex + 1]['uuid']))) ? false : disabledGroupUuidList.includes(v.uuid)">{{ $t('term.expression.and') }}</DropdownItem>
                        <DropdownItem name="or" :disabled="(disabledGroupUuidList.includes(v.uuid) && (conditionList[tIndex + 1] && !disabledGroupUuidList.includes(conditionList[tIndex + 1]['uuid']))) ? false : disabledGroupUuidList.includes(v.uuid)">{{ $t('term.expression.or') }}</DropdownItem>
                      </DropdownMenu>
                    </Dropdown>
                  </div>
                </div>
                  ...
                  <div class="pt-nm pr-nm mb-sm text-right">
                    <div class="action-group">
                      <div class="action-item">
                        <Button @click="isVisible = false">{{ $t('button.cancel') }}</Button>
                      </div>
                      <div class="action-item">
                        <Button type="primary" @click="searchConditionBtn">{{ $t('button.search') }}</Button>
                      </div>
                    </div>
                  </div>
                </Col>
          ...
          dataList: [
            {
              ...
              handlerName: this.$t('term.autoexec.modeltype'),
              config: {
                search: true,
                transfer: true,
                multiple: true,
            ...
            },
            {
              ...
              conditionModel: 'select',
              handlerName: this.$t('page.application'),
              config: {
                search: true,
                transfer: true,
                defaultValue: [],
