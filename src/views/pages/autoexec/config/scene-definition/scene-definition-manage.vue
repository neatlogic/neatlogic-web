  ...
  <div class="scene-definition-manage-box">
    ...
    <TsContain>
      <template v-slot:topLeft>
        <span class="tsfont-plus text-action" @click="addSceneDefinition">{{ $t('term.autoexec.scene') }}</span>
      </template>
...
export default {
  ...
  methods: {
    ...
    addSceneDefinition() {
      // 添加场景定义
      this.dialogTitle = this.$t('button.addtarget', {target: this.$t('term.autoexec.scene')});
    ...
    deleteSceneDefinition(row) {
      ...
      this.$createDialog({
        ...
        content: row && row.referredCount ? this.$t('page.deletetarget', {target: this.$t('term.autoexec.scene')}) : this.$t('dialog.content.deleteconfirm', {target: this.$t('term.autoexec.scene')}),
        btnType: 'error',
        'on-ok': vnode => {
    ...
    operation(row, type) {
      ...
      if (type == 'edit' && row) {
        ...
        this.isShowDialog = true;
        this.dialogTitle = this.$t('button.edittarget', {target: this.$t('term.autoexec.scene')});
