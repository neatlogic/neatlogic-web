            ...
            <template slot-scope="{ row }">
              <div :title="row.type && row.type == 'script' ? $t('term.autoexec.customtool') : $t('term.autoexec.tool')">
                ...
                <div class="operation-li">
                  ...
                  <div v-if="row.type == 'script'" class="name-title text-tip tool-des overflow" :title="row.fullCatalogName">{{ row.fullCatalogName }}</div>
                  <div v-else class="name-title text-tip tool-des overflow">-</div>
...
export default {
  ...
  async mounted() {
    ...
    //this.selectOperation = this.selectedOption || [];
