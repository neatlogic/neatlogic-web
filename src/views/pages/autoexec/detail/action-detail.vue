<template>
  <div class="action-detail">
          ...
          <span v-if="versionIsActive == 1" class="action-item">
            <Dropdown trigger="hover" placement="bottom-start">
              ...
              <DropdownMenu slot="list">
                ...
                <!-- 激活 -->
                <DropdownItem :disabled="!effectiveBaseInfoEditable" @click.native="toggleAction">
                  <div v-if="isActive" class="more-action">{{ $t('page.disable') }}</div>
                  <div v-else class="more-action">{{ $t('page.enable') }}</div>
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
          </span>
    ...
    <TsDialog
      ...
      :hasHeader="false"
      :showCloseIcon="false"
      @on-cancel="showReleaseNewVersionDialog = false"
...
export default {
    ...
    async getAction(isFirst) {
      ...
      await this.$api.autoexec.action.getActionBasicInfo(param).then(res => {
        if (res.Status == 'OK') {
          ...
          if (result.specifyVersionId) {
            specifyVersionId = result.specifyVersionId;
          } else {
            specifyVersionId = result.activeVersionId;
          }
        }
      });
      if (this.versionId == null) {
        this.versionId = specifyVersionId;
      }
      if (this.versionId != null) {
  ...
  computed: {
    ...
    effectiveCreateable() {
      return this.editable == 1 && this.versionIsActive == 1;
    },
    effectiveCancelable() {
