            ...
            <template v-slot:action="{ row }">
              <div class="tstable-action">
                <ul class="tstable-action-ul">
                  ...
                  <li class="tsfont-trash-o" @click="deleteCustomTemplate(row.id)">{{ $t('button.delete') }}</li>
                </ul>
              </div>
            </template>
          </TsTable>
  ...
  methods: {
    deleteCustomTemplate(id) {
      this.$createDialog({
        ...
        content: this.$t('dialog.content.deleteconfirm', {target: this.$t('term.autoexec.customtemplate') }),
        'on-ok': vnode => {
