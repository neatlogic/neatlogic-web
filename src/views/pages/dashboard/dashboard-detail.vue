    ...
    <TsContain border="border" :hasContentPadding="false">
      ...
      <template v-slot:topRight>
        <div v-if="dashboard" class="action-group" style="text-align:right">
          <span v-if="dashboard.type == 'custom' && dashboard.fcu == $AuthUtils.getCurrentUser().uuid" class="action-item tsfont-trash-o" @click="delDashboard">删除</span>
          <span v-if="(dashboard.type == 'system' && $AuthUtils.hasRole('DASHBOARD_MODIFY')) || (dashboard.type == 'custom' && dashboard.fcu == $AuthUtils.getCurrentUser().uuid)" class="action-item ts-setting" @click="editDashboard">编辑</span>
    ...
    delDashboard() {
      ...
      if (this.id) {
        this.$createDialog({
          ...
          content: `确定删除仪表板“${this.dashboard && this.dashboard.name}”？`,
          btnType: 'error',
          'on-ok': vnode => {
