<template>
  <div
    class="knowledge-document-editor-block-menu"
    @click.stop
    @mouseenter="$emit('menu-hover', true)"
    @mouseleave="$emit('menu-hover', false, $event)"
  >
    <EmptyRowMenu
      v-if="isEmptyRow"
      class="menu-wrapper"
      :style="{
        position: 'absolute',
        top: menuPosition.top + 'px',
        left: menuPosition.left + 'px'
      }"
      @click-menu="(menuData)=> {
        $emit('insert-menu-content', menuData)
      }"
      @dropdown-visible-change="status => $emit('dropdown-visible-change', status)"
    >
    </EmptyRowMenu>
    <EditRowMenu
      v-else
      :style="{
        position: 'absolute',
        top: (menuPosition.top) + 'px',
        left: menuPosition.left + 'px'
      }"
      :node-config="nodeConfig"
      @click-menu="(menuData)=> {
        $emit('replace-menu-content', menuData)
      }"
      @insert-below-position="(menuData)=> {
        $emit('insert-below-position', menuData)
      }"
      @handleMouse="handleMouse"
      @drag-start="event => $emit('drag-start', event)"
      @dropdown-visible-change="status => $emit('dropdown-visible-change', status)"
    >
    </EditRowMenu>
  </div>
</template>
<script>
export default {
  name: '',
  components: {
    EmptyRowMenu: () => import('@/views/pages/knowledge/category/knowledgeeditor/menus/block-menu/empty-row-menu/index.vue'),
    EditRowMenu: () => import('@/views/pages/knowledge/category/knowledgeeditor/menus/block-menu/edit-row-menu/index.vue')
  },
  props: {
    isEmptyRow: {
      type: Boolean,
      default: false
    },
    menuPosition: {
      type: Object,
      default: () => {
        return {
          top: 0,
          left: 0
        };
      }
    },
    nodeConfig: {
      type: Object,
      default: () => {
        return {};
      }
    }
  },
  data() {
    return {
    };
  },
  beforeCreate() {},
  created() {},
  beforeMount() {},
  mounted() {},
  beforeUpdate() {},
  updated() {},
  activated() {},
  deactivated() {},
  beforeDestroy() {},
  destroyed() {},
  methods: {
    handleMouse(status) {
      this.$emit('handleMouse', status);
    }
  },
  filter: {},
  computed: {},
  watch: {}
};
</script>
<style lang="less" scoped>
</style>
