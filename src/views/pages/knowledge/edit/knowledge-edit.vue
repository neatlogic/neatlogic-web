    ...
    <TsContain border="border">
      ...
      <template slot="topRight">
        <div class="action-group" style="text-align:right">
          ...
          <span class="action-item tsfont-send" :class="{disable:disabledBtn.submitDocument}" @click="submitDocument">{{ $t('term.knowledge.submitaudit') }}</span>
  ...
  methods: {
    ...
    saveDraftDocument(type) {
      ...
      return new Promise((resolve, reject) => {
        ...
        //   if (type) {
        //     this.$Notice.success({ title: this.$t('message.content.savesuccess'), duration: 1.5 });
        //   }
        //   resolve(data);
        // } else {
    ...
    async submitDocument() {
      ...
      try {
        ...
        this.$Message.success(this.$t('message.content.executesuccess')); //操作成功
        let config = res.Return;
      ...
      if (this.knowledgeDocumentId && this.isReviewer == 1 && this.title != this.defaultConfig.title) {
        if (this.title != '' && this.$utils.nameRegularValid(this.title)) {
          ...
          this.$Notice.error({
            ...
            desc: this.$t('message.content.required', {target: this.$t('page.title')}),
            duration: 1.5
          });
          this.$nextTick(() => {
            this.$refs.titleInput.focus();
