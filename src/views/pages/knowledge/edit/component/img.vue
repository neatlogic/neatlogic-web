    ...
    <div class="wrapper" :class="align">
      <div class="img">
        <div class="top tool bg-op shadow">
          <div title="居左" @click="setAlign('left')"><i class="tsfont-horizontal-left text-action" :class="{'text-primary' : align=='left'}"></i></div>
          <div title="居中" @click="setAlign('center')"><i class="tsfont-horizontal-center text-action" :class="{'text-primary' : align=='center'}"></i></div>
          <div title="居右" @click="setAlign('right')"><i class="tsfont-horizontal-right text-action" :class="{'text-primary' : align=='right'}"></i></div>
          <div title="删除" class="delete" @click="removeItem"><i class="tsfont-trash-s text-action"></i></div>
        </div>
        <div v-if="isError" class="img-loading bg-op text-error">图片加载失败</div>
        <div v-else-if="loading || !config.url" class="img-loading bg-op"><span class="loading ivu-icon ivu-icon-ios-loading"></span>正在加载图片</div>
        <img
          v-if="config.url"
          ref="img"
          :src="imgSrc"
