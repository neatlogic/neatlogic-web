<template>
  <div>
    <div class="select-template">
      ...
      <div class="select border-color text-action" @click="showTemplate = !showTemplate;getTemplateList()">  {{ $t('page.selecttarget',{target:$t('page.template')}) }}</div>
    </div>
    <TsDialog
      :isShow="showTemplate"
      :title="$t('page.selecttarget',{target:$t('page.template')})"
      width="60%"
    ...
    >
      ...
      <template slot="footer">
        ...
        <Button type="primary" @click="saveOk">{{ $t('button.confirm') }}</Button>
      </template>
    </TsDialog>
  </div>
</template>
