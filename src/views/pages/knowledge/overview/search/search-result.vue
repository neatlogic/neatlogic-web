<template>
  <div class="search-result knowledge-search">
    <p v-if="currentPath" class="result-count">
      ...
      <span class="text-grey">（{{ $t('term.knowlege.searchresult',{target: result.rowNum}) }}）</span>
    </p>
        ...
        <ul class="result-list">
          ...
          >
            <div class="flex-between result-top">
              <div>
                <span v-if="knowledge.sourceName" :title="knowledge.sourceName" class="status-name bg-warning-grey">转</span>
              ...
              <ul class="action-group">
                <li v-if="knowledge.isEditable" class="tsfont-edit action-item" @click.stop="toEditKnowledge(knowledge)">{{ $t('button.edit') }}</li>
                <li v-if="knowledge.isDeletable" class="tsfont-trash-s action-item" @click.stop="deleteKnowledge(knowledge)">{{ $t('button.delete') }}</li>
                ...
                <li
                  ...
                  class="action-item"
                  :class="knowledge.isCollect == 1 ? 'text-href' : 'text-tip'"
                  @click.stop="toggleCollect(knowledge)"
                >
                  <i class="tsfont-star" :class="knowledge.isCollect == 1 ? 'text-href' : 'text-tip'"></i>
            ...
            <div class="user-lcu text-tip">
              ...
              <span class="lcd">于{{ knowledge.lcd | formatDate }}</span>
...
export default {
  ...
  updated() {},
  activated() {
    document.title = this.knowledgeTypeName;
  },
  deactivated() {},
  beforeDestroy() {},
  destroyed() {},
  ...
  computed: {
    ...
    getText() {
      return this.knowledgeType === 'all' && this.circleListNum <= 0 ? '您不属于任何知识圈，请联系管理员处理' : '暂无数据';
    },
    setHeight() {
