  ...
  methods: {
    async init(operation) {
      if (operation === 'add') {
        ...
        const promise = this.$api.knowledge.circle.getCircle(params);
    ...
    async saveCircle(stay = true) {
      ...
      const res = await this.$api.knowledge.circle.saveCircle(params).finally(res => {
      ...
      if (res.Status === 'OK') {
        ...
        if (this.$route.query.operation === 'add') {
          stay && this.$router.push({name: 'circle-edit-dialog', query: {operation: 'edit', id: res.Return.id}});
    ...
    async deleteCircle() {
      ...
      this.$createDialog({
        ...
        'on-ok': async vnode => {
          ...
          const params = { id };
          const res = await this.$api.knowledge.circle.deleteCircle(params).finally(res => {
