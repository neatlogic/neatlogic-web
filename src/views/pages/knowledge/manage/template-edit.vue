            ...
            <template v-slot:right="{node, index, parentNode, level}">
              <div class="btn-list">
                <ul class="action-group">
                  ...
                  <li class="action-item text-action" :class="{'disabled text-grey' : level >= treeDepth}" @click="addNode(node, node.children.length, level + 1)"><i class="tsfont-plus">{{ $t('term.knowledge.lowerdirectory') }}</i></li>
                  <li class="action-item text-action" @click="removeNode(parentNode, index)"><i class="tsfont-trash-s">{{ $t('page.delete') }}</i></li>
                </ul>
              </div>
            </template>
          </TsTree>
...
class Node {
  constructor({handler, uuid, content, config, children} = {}) {
    ...
    this.content = content || Vue.prototype.i18n.t('page.catalogue');
    this.config = config || {};
  ...
  methods: {
    ...
    async saveTemplate(stay = true) {
      ...
      if (this.rootNode.content === '') {
        this.$Notice.warning({title: this.$t('message.content.required', {target: this.$t('page.templatename')})});
        this.$refs['input' + this.rootNode.uuid].$el.querySelector('input').focus();
        return false;
      }
      if (this.rootNode.children.length === 0) {
        this.$Notice.warning({title: this.$t('message.content.required', {target: this.$t('term.knowledge.navigationdirectory')})});
        return false;
      }
      const params = {
