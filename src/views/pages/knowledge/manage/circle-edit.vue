            ...
            <template v-slot:right="{node, index, parentNode, level}">
              <div class="btn-list">
                <ul class="action-group">
                  ...
                  <li class="action-item text-action" @click="addNode(parentNode, index + 1, level)"><i class="tsfont-plus">{{ $t('term.knowledge.samedirectory') }}</i></li>
                  <li class="action-item text-action" @click="addNode(node, node.children.length, level + 1)"><i class="tsfont-plus">{{ $t('term.knowledge.lowerdirectory') }}</i></li>
                  <li class="action-item text-action" :class="{'text-grey text-disabled': node.documentCount > 0}" @click="removeNode(node, parentNode, index)"><i class="tsfont-trash-s">{{ $t('page.delete') }}</i></li>
                </ul>
              </div>
            </template>
          </TsTree>
  ...
  methods: {
    async init(operation) {
      if (operation === 'add') {
        ...
        this.knowledgeCircle.children.push({
          uuid: utils.setUuid(),
          parentUuid: 0,
          name: this.$t('page.newtarget', {target: this.$t('term.knowledge.classify')}),
          children: []
        });
    ...
    async saveCircle(stay = true) { 
      if (this.knowledgeCircle.children.length === 0) {
        this.$Notice.warning({title: this.$t('message.content.required', {target: this.$t('term.knowledge.knowtype')})});
        return false;
      }
      if (!this.$refs.form.valid()) {
        this.$Notice.warning({title: this.$t('message.content.required', {target: this.$t('term.knowledge.approver')})});
        return false;
      }
      ...
      if (nameArray.length !== nameSet.size) {
        this.$Notice.warning({title: this.$t('message.content.cannotrepeat', {target: this.$t('term.knowledge.knowtypename')})});
        return false;
      }
      const params = {
