  ...
  <div>
    <TsContain :enableCollapse="true">
      ...
      <template slot="topLeft">
        <div>
          <div v-if="searchParam.interfaceId && searchParam.corporationId" class="action-group">
            <div class="action-item tsfont-plus" @click="addInterfaceItem()">数据</div>
            <div class="action-item tsfont-upload" @click="showImportDialog()">导入数据</div>
            <div v-download="exportData()" class="action-item tsfont-download">导出数据</div>
            <div v-if="hasSelectItem" class="action-item tsfont-trash-o" @click="batchDelInterfaceItem">批量删除</div>
      ...
      <template v-slot:content>
            ...
            >
                      ...
                      <tr v-for="(interfaceItem, index) in interfaceItemData.tbodyList" :key="index">
                        <td>
                          <Dropdown placement="bottom-start" :transfer="true">
                            ...
                            <DropdownMenu slot="list">
                              <DropdownItem @click.native="editInterfaceItem(interfaceItem.id)">编辑</DropdownItem>
                              <DropdownItem @click.native="validInterfaceItem(interfaceItem.id)">校验</DropdownItem>
                              <DropdownItem v-if="!interfaceItem.isImported" @click.native="delInterfaceItem(interfaceItem.id)">删除</DropdownItem>
                            </DropdownMenu>
                          </Dropdown>
                        </td>
                        ...
                        <td v-for="(prop, pindex) in allPropertyList" :key="pindex">
                          ...
                          <div v-else-if="interfaceItem.dataText && prop.complexId && interfaceItem.dataText[prop.complexId] && interfaceItem.dataText[prop.complexId].length > 0">
                            ...
                            <div v-if="!isShowMore(interfaceItem.id, prop.complexId) && interfaceItem.dataText[prop.complexId].length > 5" class="text-href" @click="showMore(interfaceItem.id, prop.complexId)">查看更多...</div>
                            <div v-if="isShowMore(interfaceItem.id, prop.complexId) && interfaceItem.dataText[prop.complexId].length > 5" class="text-href" @click="hideMore(interfaceItem.id, prop.complexId)">收起</div>
                          </div>
              ...
              <div v-if="interfaceItemData.pageCount > 1" class="tstable-page text-right">
                <ul class="ivu-page mini">
                  <span class="ivu-page-total">共 {{ interfaceItemData.rowNum }} 条</span>
                  <li
                  ...
                  <li v-else title="上一页" class="ivu-page-prev ivu-page-disabled">
                    <a><i class="ivu-icon ivu-icon-ios-arrow-back"></i></a>
                  </li>
                  <li
                  ...
                  <li v-else title="下一页" class="ivu-page-next ivu-page-disabled">
                    <a><i class="ivu-icon ivu-icon-ios-arrow-forward"></i></a>
                  </li>
                </ul>
              </div>
        ...
        <div v-else><NoData>请选择接口</NoData></div>
      </template>
    </TsContain>
    <InterfaceItemEdit
