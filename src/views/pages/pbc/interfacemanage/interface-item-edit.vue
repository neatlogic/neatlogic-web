  ...
  <div>
    <TsDialog v-bind="dialogConfig" @on-close="close">
      <template v-slot:header>
        <div v-if="interfaceItemData.id">编辑数据</div>
        <div v-if="!interfaceItemData.id">添加数据</div>
      </template>
      <template v-slot>
        <div class="form-main">
                    ...
                    <tbody v-else class="tbody-main">
                      <tr>
                        <td :colspan="prop.subPropertyList.length + 1">
                          <span class="tsfont-plus text-href" @click="addSubItem(prop.complexId)">添加数据项</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
