    ...
    <TsDialog v-bind="dialogConfig" @on-close="close">
      <template v-slot>
        <div v-if="propertyRelData.tbodyList.length == 0" style="text-align:center">
          <a href="javascript:void(0)" @click="addRel()">添加关系</a>
        </div>
        <div v-else>
          <TsTable
...
export default {
  ...
  data() {
    return { 
      ...
      theadList: Object.freeze([
        { title: '接口', key: 'toInterfaceId', width: '33%' },
        { title: '值属性', key: 'toValuePropertyId', width: '33%' },
        {title: '名称属性', key: 'toTextPropertyId', width: '33%'}
      ]),
      propertyRelData: {
        theadList: [
          {key: 'toInterfaceId', title: '接口'},
          {key: 'toValuePropertyId', title: '值属性'},
          {key: 'toTextPropertyId', title: '名称属性'},
          {key: 'btn'}
        ],
        tbodyList: [{}]
      }
  ...
  methods: {
    getPropertyRelByPropertyUid() {
      if (this.propertyUid) {
        this.$api.pbc.property.getPropertyRelByPropertyUid(this.propertyUid).then(res => {
    ...
    save() {
      ...
      if (isValid) {
        this.$api.pbc.property.savePropertyRel({uid: this.propertyUid, relList: this.propertyRelData.tbodyList}).then(res => {
          if (res.Status == 'OK') {
            this.close(true);
