      ...
      <div slot="topRight" style="text-align:right">
        <div class="bar-top-right action-group">
          ...
          <template v-if="matrixType == 'custom'">
            <div class="action-item tsfont-plus" @click="addMatrixAttribute">字段</div>
            <div v-if="processMatrixAttributeList && processMatrixAttributeList.length > 0" class="action-item tsfont-plus" @click="editMatrixData()">行</div>
            <div v-else class="action-item tsfont-plus disable" title="请先添加字段数据">行</div>
          </template>
          <template v-else>
            <div class="action-item tsfont-plus">数据</div>
          </template>
  ...
  methods: {
    ...
    deleteMatrixData: function(val, type) {
      ...
      this.$createDialog({
        ...
        content: this.matrixDataSelectList.length === 1 ? '确定删除该行数据？' : '确定删除选中数据？',
        btnType: 'error',
        'on-ok': vnode => {
