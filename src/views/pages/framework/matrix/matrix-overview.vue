<template>
  <div class="matrix-overview">
    ...
    <TsContain>
      <template v-slot:topLeft><span class="tsfont-plus text-action" @click="addMatrix">添加矩阵</span></template>
            ...
            <template slot="action" slot-scope="{ row }">
              <div class="tstable-action">
                <ul class="tstable-action-ul">
                  <li v-if="row.type === 'custom'" class="ts-intersect icon" @click.stop="copyMatrix(row)">复制</li>
                  <li
                  ...
                  <li v-if="row.type === 'custom'">
                    <Dropdown trigger="click" transfer>
                      ...
                      <DropdownMenu slot="list" class="dropdown">
                        <DropdownItem>
                          <div @click="exportMatrix(row.uuid, row.name)">导出</div>
                        </DropdownItem>
                        <DropdownItem>
                          <div @click="exportAttributeMatrix(row.uuid)">导出模板</div>
                        </DropdownItem>
                        <DropdownItem>
                          <Upload
                            ref="upload"
          ...
          <div class="editIntegration">
            <TsFormSelect v-model="addAtrixForm.integrationUuid.value" v-bind="addAtrixForm.integrationUuid">
              <template v-slot:first-ul>
                <li class="tsfont-plus text-href first-slot" @click="addIntegration()">配置</li>
              </template>
            </TsFormSelect>
            <div class="action-group action-list">
              <span class="action-item tsfont-rotate-right" title="刷新" @click="refreshIntegrationUuidList()"></span>
              <span
          ...
          <div>
            ...
            <div>
              ...
              <div class="text-tip tips desc-text-mt">
                配置文件为XML文件，通过SQL语句抽取系统内部任意数据表的数据生成配置项模型，虚拟模型中的数据不能修改和删除，但可以被其他模型关联或直接查询。
                <Poptip
                  trigger="hover"
                  placement="right"
                  width="500"
    ...
    return {
      ...
      addAtrixForm: {
        ...
        label: {
          ...
          width: '100%',
          label: '唯一标识',
          validateList: ['required', 'char', { name: 'searchUrl', url: 'api/rest/matrix/save', message: '矩阵唯一标识已存在', key: 'label' }]
        },
        // cmdbId: {
        //   type: 'radio',
        //   label: '视图类型',
      ...
      this.$api.framework.matrix.getMatrixList(data).then(res => {
        if (res.Status == 'OK') {
          ...
          this.matrixCardData.cardList.map(v => {
            ...
            v.btnList = [
              {name: '引用', value: 'ReferenceSelect', icon: '', type: 'ReferenceSelect', calleeType: 'matrix'},
              {name: '删除', value: 'del', type: 'del', icon: 'tsfont-trash-o', disable: true, text: v.type === 'private' ? '当前矩阵为私有数据源，无法删除' : '当前矩阵被引用，无法删除', key: 'disable'}
            ];
            
            if (v.type == 'custom') {
              v.btnList.push(
                {name: '多个', value: 'dropdown', icon: '', type: 'dropdown', menuArr: 
                  [{name: '复制', value: 'copy', type: 'text'}, {name: '导出', value: 'export', type: 'download'}, 
                    {name: '导出模板', value: 'exportAttr', type: 'text'}],
