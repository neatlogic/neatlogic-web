            ...
            <template v-slot:integrationUuid>
              <div>
                ...
                <div class="action-group action-list">
                  <span class="action-item tsfont-rotate-right" title="刷新" @click="getIntegrationUuidList(true)"></span>
                  <span v-if="isDisabledEditMatrix" class="action-item tsfont-edit text-disabled" title="当前矩阵已被表单引用，无法修改集成设置"></span>
                  <template v-else>
                    <span
  ...
  async created() {
    ...
    for (let i in this.formSetting) {
      if (i == 'integrationUuid') {
        ...
        this.formSetting[i].disabledHoverTitle = isDisabled ? '当前矩阵已被表单引用，无法修改集成设置' : '';
      }
    }
