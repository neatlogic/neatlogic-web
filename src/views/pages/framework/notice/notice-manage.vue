        ...
        <ul class="notice-list">
          <li v-for="notice in tableConfig.tbodyList" :key="notice.id" class="notice flex-between block-container">
            <div :style="{'width': '380px'}">
              ...
              <div class="fcu">
                <span class="text-title mr-md">创建人</span>
                <UserCard v-bind="notice.fcuVo" hideAvatar class="usercard" />
              </div>
              <div class="recipients">
                <span class="text-title mr-md">通知对象</span>
                <UserCard
            ...
            <div :style="{'width': '330px'}">
              <div class="time" :style="{'margin': '0 50px 0 50px'}">
                <template v-if="notice.startTime && notice.endTime">
                  <p class="text-title" :style="{'margin-bottom': '2px'}">下发时段</p>
                ...
                <template v-else-if="notice.startTime && !notice.endTime">
                  <p class="text-title" :style="{'margin-bottom': '5px'}">下发时段</p>
                  <span>从 </span>
                  <span>{{ notice.startTime | formatDate }}</span>
                  <span> 开始</span>
                </template>
              ...
              <ul class="action-list">
                <li
                  ...
                  title="停用后，公告将从消息列表消失"
                  @click="stop(notice)"
                >停用</li>
                <li v-else class="action-item text-action tsfont-send" @click="issue(notice)">下发</li>
