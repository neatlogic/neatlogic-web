          ...
          <template v-slot:action="{ row }">
            <div class="tstable-action">
              <ul class="tstable-action-ul">
                <li :class="row.status == 'doing'?'disabled':''" class="tsfont-restart" @click="rebuildIndex(row)">重建索引</li>
              </ul>
            </div>
          </template>
        </TsTable>
  ...
  data() {
    return {
      ...
      fullTextIndexRebuildAuditData: {
        theadList: [{key: 'typeName', title: '类型'},
          {key: 'statusText', title: '状态'},
          {key: 'indexCount', title: '索引数量'},
          {key: 'startTime', title: '重建开始时间', type: 'time'},
          {key: 'endTime', title: '重建结束时间', type: 'time'},
          {key: 'error', title: '异常'},
          {key: 'action'}]
      },
