        ...
        <div class="card-item bg-op block-container">
          ...
          <div class="btn-list">
            <ul class="action-group">
              <li class="action-item tsfont-edit" @click="editTeam(false, 'teamTabs', cd, true)">{{ $t('button.edit') }}</li>
              <li class="action-item tsfont-plus" @click="editTeam(false, 'userTabs', cd)">{{ $t('page.user') }}</li>
              <li class="action-item tsfont-plus" @click="editTeam(true, 'teamTabs', cd, false)">下级分组</li>
              <li class="action-item tsfont-trash-o" @click="delTeam(cd.uuid, cd.name, cd)">{{ $t('button.delete') }}</li>
            </ul>
          </div>
        </div>
...
export default {
  ...
  created() {
    if (this.search) {
      // console.log(this.childrenList[0].name)
      // this.$set(this.childrenList[0], "isShow", true);
      // this.$set(this.childrenList[0], "isRequest", true);
    } else {
  ...
  methods: {
          ...
          this.$api.framework.team
            ...
            .then(res => {
              if (res.Status == 'OK') {
                ...
                // console.log(_this.childrenList,'_this.children');
                let obj = _this.childrenList.find(el => el.uuid === id);
    ...
    addDrag(evt) {
      ...
      // this.$set(this, 'addParentUuid', obj.attributes.getNamedItem("uuid").value);
    },
    ...
    updataparent() {
      //   console.log(this.$root, 'ssssssss');
    },
    setItem(item) {
