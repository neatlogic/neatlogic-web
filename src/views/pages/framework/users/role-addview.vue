  ...
  <div class="role-addview">
    <TsContain>
      ...
      <template v-slot:topLeft>
        <span class="block-item">{{ uuid==null?'新建角色':'编辑角色' }}</span>
      </template>
      <div slot="topRight" class="top">
        <div class="bar-top">
          <div class="bar-top-right">
            <div v-if="uuid == null">
              <Button v-show="current > 0" type="default" @click="current -= 1">上一步</Button>
              <Button v-show="current != 3" type="primary" @click="next(current+1)">下一步</Button>
              <Button
                v-show="current == 3"
                type="primary"
                :loading="isLoading"
      ...
      <div slot="content" class="content">
        <div v-if="uuid == null">
          ...
          <div v-show="current == 0" class="form">
            ...
            <Button v-show="current != 3 && current == 0" type="primary" @click="next(current+1)">下一步</Button>
          </div>
    ...
    >
      <div class="submitModelBox">
        <div class="submit-model-title">
          <Icon type="i-icon ts-check" size="30" class="text-op" />创建成功
        </div>
        <div class="submit-btn-list">
          <Button size="large" type="primary" @click="$back(),saveProfile('backrolelist')">返回角色列表</Button>
        </div>
        <div class="submit-btn-list">
          <Button size="large" @click="continueCreate(),saveProfile('keeponcreate')">继续创建</Button>
        </div>
        <div class="submit-btn-list">
          <Button size="large" @click="editCrrentRole(),saveProfile('editrole')">编辑角色</Button>
        </div>
        <div class="submit-btn-list">
          <Checkbox v-model="submitMessage">不再提示 <i class="ts-m-problem" title="可在个人设置中修改"></i></Checkbox>
        </div>
      </div>
    </TsDialog>
  </div>
    ...
    return {
      formData: [
        ...
        },
        {
          ...
          maxlength: 50,
          label: 'ID',
          validateList: [{ name: 'required', message: '请输入角色ID' }, { name: 'name-special' }],
          errorMessage: '',
          onChange: value => {
            _this.formData[1].errorMessage = '';
            _this.allroleList.forEach(item => {
              if (item.name == value) {
                _this.formData[1].errorMessage = '角色ID已存在'; 
                this.roleKey = false;
              }
            });
          }
        },
        {
          ...
          maxlength: 50,
          label: '角色名',
          validateList: [{ name: 'required', message: '请输入角色名称' }, { name: 'name-special' }]
        }
      ],
      uuid: '', //角色uuid
      roleId: '', //角色id
    ...
    getUserNum: function(val) {
      ...
      this.$api.framework.user.searchUser(data).then(res => {
        if (res.Status == 'OK') {
          ...
          this.userNum = num || '';
          this.label1 = this.getLabel('基本信息', 'role');
          this.label4 = this.getLabel('授权', 'auth');
          this.label2 = this.getLabel('成员列表' + (this.userNum ? '(' + this.userNum + ')' : ''), 'adduser');
          this.label3 = this.getLabel('分组列表', 'addgroup');
        }
      });
    },
    //获取用户授权列表
