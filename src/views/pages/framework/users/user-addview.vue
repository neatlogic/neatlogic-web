  ...
  <div class="user-addview">
    <TsContain border="border">
      ...
      <template v-slot:topLeft>
        <span>{{ uuid && readonly?'查看用户':(uuid && !readonly?'编辑用户':'新建用户') }}</span>
      </template>
        ...
        <div class="bar-top">
          <div class="bar-top-right">
            <div v-if="uuid == null">
              ...
              <Button v-show="current != 1" type="primary" @click="next()">下一步</Button>
              <Button
                v-show="current == 1"
                type="primary"
                :loading="isLoading"
    ...
    >
      <div class="submitModelBox">
        <div class="submit-model-title">
          <i class="tsfont-check-s text-success"></i>创建成功
        </div>
        <div class="submit-btn-list">
          <Button size="large" type="primary" @click="$back(), saveProfile('backuserlist')">返回用户列表</Button>
        </div>
        <div class="submit-btn-list">
          <Button size="large" @click="continueCreate(), saveProfile('keeponcreate')">继续创建</Button>
        </div>
        <div class="submit-btn-list">
          <Button size="large" @click="editCrrentUser(), saveProfile('edituser')">编辑用户</Button>
        </div>
        <div class="submit-btn-list">
...
export default {
    ...
    return {
      formData: [
        ...
        },
        {
          ...
          width: '100%',
          label: 'ID',
          validateList: [{ name: 'required', message: '请输入Id' }, { name: 'name-special' }, {
            name: 'searchUrl',
        ...
        },
        {
          ...
          width: '100%',
          label: '用户邮箱',
          validateList: [{ name: 'mail', message: '请输入正确邮箱' }]
        },
        {
          type: 'tel',
          name: 'phone',
          ...
          label: '手机号',
          validateList: [
            { name: 'number', message: '请正确输入手机号' },
            { name: 'phone', message: '请输入正确的手机号码' }
          ]
        },
        {
          type: 'switch',
  ...
  created() {
    ...
    this.label1 = this.getLabel('基本信息', 'user');
    this.label2 = this.getLabel('授权', 'auth');
    this.getRoleList();
    this.getAuthList();
    this.getProfile();
  },
  ...
  methods: {
    ...
    next() {
      ...
      if (data.password !== data.confirmpwd) {
        this.$Message.error('两次密码前后输入不一致');
        this.stepStatus = 'error';
        return;
      }
