  ...
  <div class="team-addview">
    <TsContain>
      ...
      <template v-slot:topLeft>
        <span class="block-item">{{ isAdd? '新建分组':'编辑分组' }}</span>
      </template>
        ...
        <div class="bar-top">
          <div class="bar-top-right">
            <div v-if="isAdd">
              ...
              <Button v-show="current < 1" type="primary" @click="next(current+1)">下一步</Button>
              <Button
                v-show="current == 1"
                type="primary"
                :loading="isLoading"
        ...
        <div v-if="isAdd">
          ...
          <div v-show="current == 0" class="form">
            <BaseDetail
              ...
              :isAdd="isAdd"
            ></BaseDetail>
            <Button v-show="current != 2 && current == 0" type="primary" @click="next(current+1)">下一步</Button>
          </div>
    ...
    >
      <div class="submitModelBox">
        ...
        <div class="submit-btn-list">
          <Button size="large" type="primary" @click="$back(),saveProfile('backteamlist')">返回用户组列表</Button>
        </div>
        <div class="submit-btn-list">
          <Button size="large" @click="continueCreate(),saveProfile('keeponcreate')">继续创建</Button>
        </div>
        <div class="submit-btn-list">
          <Button size="large" @click="editCrrentTeam(),saveProfile('editteam')">编辑用户组</Button>
        </div>
        <div class="submit-btn-list">
          <Checkbox v-model="submitMessage">不再提示</Checkbox>
        </div>
      </div>
    </TsDialog>
    <TsDialog
    ...
    getUserNum: function(val) {
      ...
      this.$api.framework.user.searchUser(data).then(res => {
        if (res.Status == 'OK') {
          ...
          this.label1 = this.getLabel('基本信息', 0, 'teamTabs');
          this.label2 = this.getLabel('成员列表', num, 'userTabs');
        }
      });
    },
    //tabs点击
