      ...
      <div class="bar-top">
        <div class="bar-top-left">
          <div class="div-btn-contain action-group">
            <template>
              <span v-show="type !== 'auth_role'" class="action-item tsfont-plus" @click="add(type)">新增成员</span>
              <span v-show="type == 'auth_role'" class="action-item tsfont-plus" @click="add(type)">新增角色</span>
            </template>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    getRoleSelect(list) {
      ...
      this.roleCount = {
        text: list.length > 0 ? `已选中${list.length}个角色` : '',
