    ...
    <template v-slot>
      <div>
        <div v-if="Object.keys(helpData).length">
          ...
          <div>{{ rowData.url }}</div>
          <Divider v-if="helpData.description" orientation="left">描述</Divider>
          ...
          <Divider v-if="testData.result" orientation="left">输出结果</Divider>
          <JsonViewer
...
export default {
  ...
  data() {
    return {
      ...
      requestType: [
        { value: 'form', text: '表单模式' },
        { value: 'json', text: 'json模式' }
      ],
      tab: 'form',
      helpData: {},
      testData: {},
      ...
      inputColumns: Object.freeze([
        ...
        { title: '类型', key: 'type' },
        { title: '是否必填', key: 'isRequired', width: 100, render: (h, params) => {
