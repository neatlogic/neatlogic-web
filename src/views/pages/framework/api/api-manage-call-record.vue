        ...
        <template v-slot:action="{row}">
          <div class="tstable-action">
            <ul class="tstable-action-ul">
              <li v-if="row.paramFilePath" @click="getCallDetail(row.paramFilePath,'参数')">参数</li>
              <li v-if="row.resultFilePath" @click="getCallDetail(row.resultFilePath,'结果')">结果</li>
              <li v-if="row.errorFilePath" @click="getCallDetail(row.errorFilePath,'异常')">异常</li>
            </ul>
          </div>
        </template>
      </TsTable>
  ...
  data() {
    return {
      ...
      theadList: Object.freeze([
        { title: '地址', key: 'token', width: 250 },
        { title: '用户名', key: 'userName', width: 150 },
        { title: '用户IP', key: 'ip', width: 150 },
        { title: '调用时间', key: 'startTime', width: 200 },
        { title: '耗时(毫秒)', key: 'timeCost', width: 100 },
