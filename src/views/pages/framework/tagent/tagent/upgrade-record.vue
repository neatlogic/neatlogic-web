...
<template>
  <div class="upgrade-record-wrap">
    <span class="text-action ts-timer upgrade-record-margin-right" @click="handleUpgradeRecord">升级记录</span>
    <TsDialog
      type="modal"
      :isShow="isShowDialog"
      v-bind="dialogConfig"
      ...
      detailTableConfig: {
        ...
        theadList: [
          ...
          },
          {
            ...
            type: 'action',
            operations: [
              {icon: 'tsfont-file-single', name: '查看日志', action: 'view', type: 'text'}]
          }
        ],
        tbodyList: [],
        ip: '', // ip
  ...
  methods: {
    getStatusList() {
      ...
      this.$https.post('/api/rest/universal/enum/get', params).then((res) => {
        let arr = [{text: '所有状态', value: 'allStatus'}, ... res.Return];
