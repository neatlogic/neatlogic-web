        ...
        <table v-if="tableConfig.rowNum > 0" class="table">
          ...
          <tbody>
            <tr v-for="(row, trIndex) in tableConfig.tbodyList" :key="trIndex" class="table-row">
              ...
              <td key="timeCost">
                <div class="table-row-wrapper bg-block">{{ row.timeCost }}毫秒</div>
              </td>
              <td key="status">
              ...
              <td key="paramFilePath">
                <div class="table-row-wrapper bg-block">
                  <span v-if="row.paramFilePath" class="text-primary text-action" @click="getAuditDetail(row.paramFilePath, '参数')">参数</span>
                  <span v-else>-</span>
                </div>
              </td>
              <td key="resultFilePath">
                <div class="table-row-wrapper bg-block">
                  <span v-if="row.resultFilePath" class="text-primary text-action" @click="getAuditDetail(row.resultFilePath, '结果')">结果</span>
                  <span v-else>-</span>
                </div>
              </td>
              <td key="errorFilePath" class="table-td-error">
                <div class="table-row-wrapper bg-block">
                  <span v-if="row.errorFilePath" class="text-primary text-action" @click="getAuditDetail(row.errorFilePath, '异常')">异常</span>
                  <span v-else>-</span>
                </div>
              </td>
            </tr>
  ...
  data() {
    return {
      ...
      theadList: [
        ...
        { key: 'apiName', title: '接口' },
        { key: 'moduleGroupName', title: '模块' },
        { key: 'timeCost', title: '耗时' },
        { key: 'status', title: '状态' },
        { key: 'paramFilePath', title: '参数' },
        { key: 'resultFilePath', title: '结果' },
        { key: 'errorFilePath', title: '异常' }
      ],
      tableConfig: {
        tbodyList: [],
        pageSize: 20,
