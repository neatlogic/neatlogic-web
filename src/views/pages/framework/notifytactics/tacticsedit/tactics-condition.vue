<template>
  <div class="tactics-condition">
    ...
    <div class="add-condition">
      <span class="tsfont-plus text-action" @click="addCondition">配置</span>
    </div>
            ...
            <div class="condition-content">
              <div v-for="(groupitem, groupindex) in item.conditionConfig.conditionGroupList" :key="groupindex" style="display: inline-block;">
                ...
                <div v-for="(conitem, conindex) in groupitem.conditionList" :key="conindex" style="display: inline-block;">
                  ...
                  <span v-if="groupitem.conditionRelList && groupitem.conditionRelList[conindex]" class="text-primary" style="margin:0 6px;">{{ groupitem.conditionRelList[conindex].joinType === 'or' ? '或' : '且' }}</span>
                </div>
                <span style="margin-left: 0px;">)</span>
                <span v-if="item.conditionConfig.conditionGroupRelList && item.conditionConfig.conditionGroupRelList[groupindex]" class="text-primary" style="margin:0 6px;">{{ item.conditionConfig.conditionGroupRelList[groupindex].joinType === 'or' ? '或' : '且' }}</span>
              </div>
            </div>
          </div>
