<template>
  <div class="formEdit" @click.stop>
            ...
            <Dropdown trigger="click">
              ...
              <DropdownMenu slot="list" class="dropdown">
                ...
                <DropdownItem v-else-if="currentVersion.uuid">
                  <div class="ts-catalogue referenceCount disable">引用列表</div>
                </DropdownItem>
    ...
    >
      <p v-if="delVersion.uuid != currentVersion.uuid">
        确定要删除
        <b>{{ delVersion.version }}版本</b>
        的表单
      </p>
      <p v-else>
        确定要删除
  ...
  methods: {
    ...
    previewForm: function() {
      ...
      if (this.previewData.controllerList && this.previewData.controllerList.length > 0) {
        let isnext = true;
        if (!isnext) {
          this.$Notice.warning({ title: '组件信息填写不完整' });
          return;
        }
      }
      //如果表单设置了组件联动
      if (this.previewData.globalSetting && this.previewData.globalSetting.ruleList && this.previewData.globalSetting.ruleList.length > 0) {
        ...
        if (!isnext) {
          this.$Notice.warning({ title: '表单设置里的组件联动填写不完整' });
          return;
        }
      }
    ...
    okExportFile: function() {
      ...
      this.$https
        ...
        .catch(error => {
          this.$Notice.error({ title: '错误信息', desc: error.data.Message });
        });
      // let data = {};
