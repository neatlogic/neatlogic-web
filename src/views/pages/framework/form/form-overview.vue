  ...
  <div class="formOverview">
    ...
    <TsContain>
      <template slot="topLeft">
        <div class="action-group">
          <span class="tsfont-plus action-item" @click="addForm">表单</span>
          <span class="tsfont-upload action-item" @click.self="$refs.uploadDialog.showDialog">导入</span>
          <UploadDialog
            ref="uploadDialog"
            :actionUrl="actionUrl"
        ...
        >
          <template slot-scope="{ row }">
            <div class="overvivew-main" :class="row.isActive == 0 ? 'isStop' : ''">
              ...
              <div class="text-title">版本：{{ row.currentVersion }}</div>
            </div>
          </template>
        </TsCard>
      </div>
    ...
    >
      <template v-slot:header>
        <div>{{ isCopy ? '复制表单' : '新建表单' }}</div>
      </template>
      <TsForm ref="mainForm" :item-list="formSetting" type="type"></TsForm>
      <template v-slot:footer>
        <div v-if="!isCopy" class="footer-btn-contain">
          <Button @click="cancelAddForm">取消</Button>
          <Button type="primary" :disabled="!canSubmit" @click="okAddForm">确定</Button>
        </div>
        <div v-else class="footer-btn-contain">
          <Button @click="cancelAddForm">取消</Button>
          <Button type="primary" :disabled="!canSubmit" @click="okCopyForm(true)">复制激活版本</Button>
          <Button type="primary" :disabled="!canSubmit" @click="okCopyForm(false)">复制全部</Button>
        </div>
      </template>
    </TsDialog>
  </div>
        ...
        .then(res => {
          if (res.Status == 'OK') {
            ...
            this.formData.cardList.map(v => {
              ...
              v.btnList = [
                { name: '引用', value: 'ReferenceSelect', icon: '', type: 'ReferenceSelect', calleeType: 'form' },
