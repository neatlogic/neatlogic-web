              ...
              >
                <template slot-scope="{ row }">
                  <div class="padding">
                    ...
                    <div v-if="row.version" class="mt-xs">
                      <span class="text-grey mr-xs">版本</span>
                      <span>{{ row.version }}</span>
                    </div>
                  </div>
                </template>
                <template v-slot:control="{ row }">
                  <div v-if="row.hasInitialData" class="tsfont-download text-action" @click="exportData(row)">导出初始数据</div>
                  <div v-if="row.hasInitialData" class="tsfont-upload text-action" @click="openImportDialog(row)">导入初始数据</div>
                </template>
              </TsCard>
            </div>
          </div>
