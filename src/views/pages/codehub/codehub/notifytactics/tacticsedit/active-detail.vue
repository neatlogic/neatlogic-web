  ...
  <div v-if="config">
    <div class="condition-text dividing-color">
      <!-- <div v-if="config.conditionConfig && config.conditionConfig.conditionGroupList && config.conditionConfig.conditionGroupList.length > 0" class="condition-block">
        <div class="text-title condition-title">条件</div>
            ...
            <div>
              ...
              <div v-for="(conitem, conindex) in groupitem.conditionList" :key="conindex" style="display: inline-block;">
                ...
                <span v-if="conitem.expression != 'is-null'">
                  <span :is="handlerType(conitem.name)" v-model="conitem.valueList" :config="getselectConfig(conitem.name)" :readonly="true" style="display: inline-block;"></span>
                </span>
                <span v-if="groupitem.conditionRelList && groupitem.conditionRelList[conindex]" class="text-primary" style="margin:0 6px;">{{ groupitem.conditionRelList[conindex].joinType === 'or' ? '或者' : '并且' }}</span>
              </div>
              <span style="margin-left: 0px;">)</span>
            </div>
            <div v-if="config.conditionConfig.conditionGroupRelList && config.conditionConfig.conditionGroupRelList[groupindex]" class="text-primary" style="margin:0 6px;">{{ config.conditionConfig.conditionGroupRelList[groupindex].joinType === 'or' ? '或者' : '并且' }}</div>
          </div>
        </div>
      </div> -->
      <div v-if="config.actionList && config.actionList.length > 0" class="condition-block">
        <div class="text-title condition-title">动作</div>
        <div class="condition-content">
