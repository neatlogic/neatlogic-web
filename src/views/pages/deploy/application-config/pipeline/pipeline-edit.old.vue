...
export default {
  ...
  components: {
    ...
    ScenarioSetting: resolve => require(['./scenario-setting'], resolve),
  ...
  methods: {
    ...
    getStepList(stepList) {
      return stepList.length ? stepList.map(l => {
        ...
        //模块层和环境层需要：inherit(继承)、override(重载)、isActive(启用),
        if (l.hasOwnProperty('inherit')) { 
          this.$set(obj, 'inherit', l.inherit);
        }
