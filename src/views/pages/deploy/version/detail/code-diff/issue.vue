...
export default {
  ...
  methods: {
    ...
    getList(forceFlush) {
      ...
      return this.$api.common.getIssuelistByMrId(param, { cancelToken: this.cancelAxios.token }).then(res => {
    ...
    handlerMr() {
      ...
      if (type == 'branch') {
        ...
        this.$api.common.mergebyBranch(param).then(res => {
    ...
    revertMr() {
      ...
      if (this.mrData.versionTypeStrategyRelationVo && this.mrData.versionTypeStrategyRelationVo.versionStrategyType && this.mrData.versionTypeStrategyRelationVo.versionStrategyType == 'branch' && this.mrData.status == 'finish') {
        ...
        this.$createDialog({
          ...
          'on-ok': (vnode) => {
            this.duringAction = true;
            this.$api.common.checkRevert(param).then((res) => {
              this.duringAction = false;
              if (res && res.Status == 'OK') {
                let param = {
    ...
    closeMr() {
      ...
      this.$api.common.updateStatus(params).then(res => {
        if (res && res.Status == 'OK') {
          this.$emit('reload');
        }
      });
    ...
    revertIssue(item) {
      ...
      this.$api.common.checkRevert(mrParam).then(res => {
        if (res && res.Status == 'OK') {
          let param = {
            issueList: [item],
            srcBranch: res.Return.srcBranch,
