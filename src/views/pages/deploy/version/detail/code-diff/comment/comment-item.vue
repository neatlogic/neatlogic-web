  ...
  methods: {
    ...
    deleteComment(item) {
      ...
      this.$createDialog({
        ...
        'on-ok': (vnode) => {
          if (item.filePath) {
            this.$api.common.deleteCommentByLine(param).then((res) => {
            ...
            this.$api.common.deleteCommentById(param).then((res) => {
              if (res && res.Status == 'OK') {
                this.$Message.success(this.$t('message.deletesuccess'));
                vnode.isShow = false;
                this.$emit('reload');
    ...
    submitReply(item) {
      if (item.filePath) {
        ...
        this.replying = true;
        this.$api.common.saveCommentByLine(param).then((res) => {
          this.replying = false;
          if (res.Status == 'OK') {
        ...
        this.replying = true;
        this.$api.common.saveComment(param).then((res) => {
          if (res.Status == 'OK') {
            this.showReply = false;
            this.commitText = '';
            this.$emit('reload');
