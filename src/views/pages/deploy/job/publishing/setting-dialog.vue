  ...
  <div>
    ...
    >
      <template v-slot>
        ...
        <template>
          <!-- 添加配置 -->
          <div v-if="selectedApp && !selectedApp.isConfig">{{ $t('term.deploy.currentapplynoconfig') }}<span class="text-href" @click="toPipeline()">{{ $t('page.addtarget', {target: $t('page.config')}) }}</span></div>
          <!-- 添加模块 -->
          <div v-else-if="selectedApp && !selectedApp.isHasModule">{{ $t('term.deploy.applynoconfigmodule') }}<span class="text-href" @click="addModule()">{{ $t('page.addtarget', {target: $t('page.module')}) }}</span></div>
          <!-- 添加环境 -->
