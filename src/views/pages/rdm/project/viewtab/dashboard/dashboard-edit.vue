          ...
          <div v-if="currentWidget">
            <Tabs>
              ...
              <TabPane v-if="currentWidgetComponent && currentWidgetComponent.dataTypes && currentWidgetComponent.dataTypes.length > 0" name="data" :label="$t('page.data')">
                <DataConfig :presetList="Object.keys(presetData)" moduleId="rdm" :widget="currentWidget"></DataConfig>
              </TabPane>
...
<script>
import { WIDGETS, WIDGET_TYPES } from '@/views/pages/dashboard/widget/widget-list.js';
    ...
    save() {
      if (this.dashboard.name) {
        if (this.dashboard.widgetList.length > 0) {
          ...
          this.dashboard.appId = this.appId;
          this.$api.rdm.dashboard.saveDashboard(this.dashboard).then(res => {
            if (res.Status == 'OK' && res.Return) {
              this.$Message.success(this.$t('message.savesuccess'));
              this.currentWidget = null;
              this.$router.push({ path: '/dashboard-edit/' + this.projectId + '/' + this.appId + '/' + res.Return });
            }
          });
        } else {
...
<style lang="less">
@import '~@/views/pages/rdm/project/viewtab/dashboard/dashboard-edit.less';
</style>
