<template>
  ...
  >
    ...
    <div ref="tableWrap" class="done-table">
      <TsTable
        :theadList="theadList"
        :tbodyList="list"
        :can-drag="false"
...
export default {
  ...
  props: {
    widget: { type: Object },
    title: { type: String, default: '我的已办' },
  ...
  methods: {
    loadData() {
      ...
      this.$api.common.searchWorkbenchWidgetData({
        handler: 'process.doneOfMineProcessTask',
        portalWidgetName: 'doneOfMineProcessTask',
        param: {
          limit: this.limit,
          needPage: false
        }
        ...
        if (!res || res.Status !== 'OK') {
          throw new Error((res && res.Message) || '我的已办加载失败');
        }
        const result = res.Return || {};
        this.sourceList = Array.isArray(result.tbodyList) ? result.tbodyList : [];
      }).catch(error => {
        this.sourceList = [];
        this.error = (error && (error.Message || error.message)) || '我的已办加载失败';
      }).finally(() => {
        this.loading = false;
        this.$nextTick(() => {
          this.updateTableHeight();
    ...
    toWorkcenter() {
      this.$router.push({ path: '/task-overview-doneOfMineProcessTask' });
    }
  },
  computed: {
    limit() {
