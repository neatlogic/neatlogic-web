<template>
  ...
  >
    ...
    <div ref="tableWrap" class="draft-table">
      <TsTable
        :theadList="theadList"
        :tbodyList="list"
        :can-drag="false"
      ...
      >
        <template v-slot:title="{ row }">
          <span class="text-href overflow" :title="getTitle(row)" @click.stop="toTask(row)">{{ getTitle(row) }}</span>
        </template>
...
export default {
  ...
  props: {
    widget: { type: Object },
    title: { type: String, default: '我的草稿' },
  ...
  methods: {
    loadData() {
      ...
      this.$api.common.searchWorkbenchWidgetData({
        handler: 'process.draftProcessTask',
        portalWidgetName: 'draftProcessTask',
        param: {
          limit: this.limit,
          needPage: false
        }
        ...
        if (!res || res.Status !== 'OK') {
          throw new Error((res && res.Message) || '我的草稿加载失败');
        }
        const result = res.Return || {};
        this.sourceList = Array.isArray(result.tbodyList) ? result.tbodyList : [];
      }).catch(error => {
        this.sourceList = [];
        this.error = (error && (error.Message || error.message)) || '我的草稿加载失败';
      }).finally(() => {
        this.loading = false;
        this.$nextTick(() => {
          this.updateTableHeight();
    ...
    toTask(item) {
      ...
      if (processTaskId) {
        this.$router.push({ path: '/task-dispatch', query: { processTaskId } });
      }
    },
    toWorkcenter() {
      this.$router.push({ path: '/task-overview-draftProcessTask' });
    }
  },
  computed: {
    limit() {
