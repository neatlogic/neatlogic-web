...
export default {
  ...
  methods: {
    ...
    changeLike: function(parent, item) {
      let _this = this;
      //如果是在收藏的查看更多中点击收藏
      if (this.modal == true && this.moreTitle == '收藏') {
      ...
      this.$api.process.service.changeLike(data).then(res => {
        if (res.Status == 'OK') {
          ...
          _this.likeItemAction = action;
          if (action == 1) {
            this.$Message.success(this.$t('message.content.collectionsuccess'));
          } else {
            this.$Message.success(this.$t('message.content.cancelsuccess'));
          }
        }
      });
    },
