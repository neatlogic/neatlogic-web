            ...
            <template slot="action" slot-scope="{ row }">
              <div class="tstable-action">
                <ul class="tstable-action-ul">
                  ...
                  <!-- <li class="tsfont-edit" :class="row.referenceCount>0?'disable':''" @click="editRelations(row)">{{ $t('button.add') }}</li> -->
                  <li class="tsfont-trash-s" :class="row.referenceCount>0?'disable':''" @click="delRelations(row)">{{ $t('button.delete') }}</li>
      ...
      relationDialogForm: [
        {
          ...
          validateList: [
            ...
            { name: 'name-special' },
            { name: 'searchUrl', url: 'api/rest/process/channeltype/relation/save', message: this.$t('form.validate.repeat', {target: this.$t('page.relname')}) }
          ]
        },
        {
          type: 'select',
