                ...
                <vuedraggable v-model="slaData.calculatePolicyList" :options="{ chosenClass: 'choose', animation: '150', handle: '.move' }">
                  <transition-group>
                    <div v-for="(item, index) in slaData.calculatePolicyList" :key="item.uuid" class="slaTime">
                      <span v-show="slaData.calculatePolicyList && slaData.calculatePolicyList.length > 1" class="tsfont-close-s btn-removerule text-tip" @click="slaDel(index)"></span>
                      <div class="block_top">
...
export default {
  ...
  methods: {
    ...
    submit: function() {
      ...
      // this.$Notice.success({
      //   title: this.$t('message.content.savesuccess'),
      //   duration: 1.5
      // });
