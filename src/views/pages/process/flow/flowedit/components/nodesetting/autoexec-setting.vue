...
export default {
  ...
  methods: {
            ...
            runtimeParamList.length > 0 && (newRuntimeParamList = runtimeParamList.map(a => {
              ...
              if (finditem) {
                ...
                if (finditem.type != a.type) {
                  _this.$set(finditem, 'value', a.value || '');
    ...
    editCombop() { //编辑合租工具
      window.open(HOME + '/autoexec.html#/action-detail?id=' + this.autoexecConfig.autoexecCombopId, '_blank');
    },
    refreshCombop() {
    ...
    paramValue(item) {
      ...
      let value = !this.$utils.isEmpty(item.value) ? this.$utils.deepClone(item.value) : '';
