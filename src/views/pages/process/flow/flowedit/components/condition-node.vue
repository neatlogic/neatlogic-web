          ...
          <div v-if="item.conditionGroupList.length >0" class="rule-content edit-setting">
            <div v-for="(groupItem,groupIndex) in item.conditionGroupList" :key="groupIndex" class="group-list">
              <div v-for="(conditionItem,conditionIndex) in groupItem.conditionList" :key="conditionIndex" class="condition-list">
                <Row :gutter="8">
                  <Col span="8">
                    <span :title="conditionShow(conditionItem.name)" class="condition-text border-color overflow">{{ conditionShow(conditionItem.name) ||'空' }}</span>
                  </Col>
                  <Col :span="conditionItem.isShowConditionValue == 1?'7':'16'">
                    <span :title="expressionShow(conditionItem.name,conditionItem.expression)" class="condition-text border-color overflow">{{ expressionShow(conditionItem.name,conditionItem.expression) || '空' }}</span>
                  </Col>
