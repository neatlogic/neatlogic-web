    ...
    <TsContain :rightWidth="340" border="border">
      ...
      <template v-slot:topRight>
        <div class="div-btn-contain action-group" style="text-align:right">
          <span class="action-item tsfont-rotate-right" @click="reset">{{ $t('button.reset') }}</span>
          <span class="action-item ts-dev-security" @click.prevent="flowDataValid">{{ $t('button.validate') }}</span>
          <span class="action-item ts-save" @click="flowSave(false)">{{ $t('button.save') }}</span>
          <span v-if="referenceCount > 0 || isNew == true" class="action-item tsfont-trash-o disable">{{ $t('button.delete') }}</span>
          <span v-else-if="referenceCount == 0 && isNew == false" class="action-item tsfont-trash-o" @click="deleteFlow">{{ $t('button.delete') }}</span>
      ...
      <template v-slot:content>
        <div class="second-contain">
          <div class="col-left-contain bg-grey col-contain tsscroll-container">
            <ul v-if="nodeList" class="node-itemul">
              <draggable v-model="nodeList" :sort="false">
                <transition-group>
                  <template v-for="(list, index) in nodeList">
                    <LeftNode v-if="!list._isHide" :key="'1'+index" :node="list"></LeftNode>
                  </template>
                </transition-group>
              </draggable>
            </ul>
          </div>
          <div class="col-center-contain col-contain tsscroll-container bg-block" @drop="drop" @dragover.prevent>
            <Card
              v-if="validCardOpen"
              class="radius-lg"
              style="width:320px;"
            ...
            >
              <p slot="title" class="text-title">{{ $t('button.validate') }}</p>
...
let viewOpts = {
  'node.selectedFn'(d, selectNodeList) {
    Vm.timeSelect && clearTimeout(Vm.timeSelect);
    Vm.timeSelect = setTimeout(() => {
      selectNodeList.length > 1 ? Vm.unSelectNode(d) : Vm.selectNode(d);
      Vm.timeSelect = null;
    }, 200);
  },
  'link.selectedFn': (d) => {
    var config = d.getConfig();
    Vm.selectTimeout && clearTimeout(Vm.selectTimeout);
    config.name = config.name || '';
    Vm.showLinkSetting(config);
  },
  'node.unselectFn': (d, selectNodeList) => {
    Vm.timeUnselect && clearTimeout(Vm.timeUnselect);
    Vm.timeUnselect = setTimeout(() => {
      selectNodeList.length == 1 ? Vm.selectNode(selectNodeList[0]) : Vm.unSelectNode(d);
      Vm.timeUnselect = null;
    }, 200);
  },
  'link.unselectFn': (d) => {
    Vm.selectTimeout = setTimeout(function() {
...
// 节点验证
function nodeValid(allNode) {
  var validList = [];
  // 验证flowsetting
  var flowSettingData = Vm.$refs.flowSetting && Vm.$refs.flowSetting.getJsonValue() ? Vm.$refs.flowSetting.getJsonValue() : '';
  if (flowSettingData) {
    validList.push({
  ...
  allNode.forEach(vm => {
    ...
    if (nodeConfig && nodeConfig.handler && nodeConfig.handler !== 'start' && nodeConfig.handler !== 'end' || !nodeConfig.handler) { //开始节点和结束节点不需要特殊校验
      //校验
      valids.push(...FlowValid.common(nodeConfig, vm, Vm));
      if (FlowValid[nodeConfig.handler] instanceof Function) {
        valids.push(...FlowValid[nodeConfig.handler](nodeConfig, vm, Vm));
      }
    }
    if (valids.length > 0) { //校验没有通
      valids.forEach(item => {
        validList.push({
          type: 'error',
          msg: `【${nodeConfig.name}】${item.name}`,
          href: item.href || '',
          focus() {
            ...
            if (nodeConfig.type === 'start' || nodeConfig.type === 'end') {
              ...
              Vm.showNodeSetting(nodeConfig, vm);
            }
            otherNodes.forEach(vm => vm.unselect());
          }
        });
      });
    } else { //校验通过
  ...
  Vm.validList = validList.sort((a, b) => {
    let a1 = a.type === 'error' ? 1 : 0;
    let b1 = b.type === 'error' ? 1 : 0;
    return b1 - a1;
  });

  let errorList = validList.length && validList.filter(d => d.type === 'error');
  return !errorList.length;
}
// 节点数据
function nodeDataTransform(data) {
  var d = data;
  if (data.chartConfig) {
    for (let dd in data.chartConfig) {
      d[dd] = data.chartConfig[dd];
    }
  }
  if (d && d.icon && d.icon.length > 1 && d.icon[0] !== '#') d.icon = '#' + d.icon;
  return d;
}
...
export default {
  ...
  created() {
    ...
    if (this.$route.query.activeTab) { //从策略页面跳转过滤，定位tab
  ...
  methods: {
    selectNodeByStepUuid() {
      // 根据步骤id选中节点并高亮
      if (this.$route.query.stepUuid && !this.$utils.isEmpty(this.stepList)) { // 从表单场景跳转过来，勾选对应的节点并且打开对应节点的编辑页面
    ...
    initTopo(config, action) {
      ...
      // 数据
      topodata.links && topodata.links.forEach(link => { //旧数据转为新类型 之后可以考虑去掉
      ...
      //步骤节点数据
      this.stepList = data.process && data.process.stepList ? data.process.stepList : topodata.nodes.map(item => { return {uuid: item.uuid, handler: item.handler, name: item.name, stepConfig: {}, type: item.type}; });
      this.stepList.forEach(item => { //给stepList塞值,由于旧的数据没有导致的问题  之后可以考虑去掉
        var list = ['type'];
        list.forEach(key => {
          if (!item[key]) {
    ...
    getFlowData(action) {
      ...
      let connectionList = topoConfig.links ? topoConfig.links.map(d => ({
        uuid: d.uuid,
    ...
    updateformitemList(list) { //更新表单组件
  ...
  watch: {
    ...
    'scoreConfig.isActive'(val) { //当评分设置关闭时需要删除结束节点的回退线
      if (!val) {
