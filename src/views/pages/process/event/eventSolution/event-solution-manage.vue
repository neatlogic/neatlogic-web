        ...
        <div class="solution-card-container">
          ...
          >
            ...
            <template v-slot:control="{ row }">
              <div class="tsfont-copy" @click="toEditPage({ id: row.id, operation: 'copy' })">{{ $t('button.copy') }}</div>
              <div class="tsfont-trash-o" @click="deleteSolution(row)">{{ $t('button.delete') }}</div>
            </template>
          </TsCard>
        </div>
      </template>
