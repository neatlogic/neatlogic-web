<template>
  <div class="taskdetail-changecreate">
      ...
      <template v-slot:content>
        <div class="task-main">
          <template v-if="(processTaskStepRemindList && processTaskStepRemindList.length > 0) || (processTaskConfig.tranferReportProcessTaskList && processTaskConfig.tranferReportProcessTaskList.length > 0)">
            <TaskAlert :list="processTaskStepRemindList" :tranferReportProcessTaskList="processTaskConfig.tranferReportProcessTaskList"></TaskAlert>
          </template>
          <TsContain
            :rightWidth="290"
          ...
          >
            <template v-slot:content>
              <div v-if="actionConfig.pocesstaskview && processTaskConfig && startProcessTaskStep">
                ...
                >
                  <template v-slot:changecreate>
                    <div v-if="startProcessTaskStep.handler != 'changecreate'" id="changeInfo">
                      <ChangecreateDispatch
                        ref="changecreateData"
                      ...
                      <div v-if="actionConfig.comment || (processTaskStepConfig && processTaskStepConfig.commentList && processTaskStepConfig.commentList.length > 0)" class="line-bottom border-color"></div>
                    </div>
                  </template>
                  <div slot="replyBtn">
                    <!-- 回退/流转按钮 -->
...
export default {
  ...
  components: {
    ...
    ChangecreateDispatch: resolve => require(['@/views/pages/process/task/processdetail/workorder/change/changecreate-dispatch.vue'], resolve),
    ChangecreateStep: resolve => require(['@/views/pages/process/task/processdetail/workorder/change/changecreate-detail'], resolve),
  ...
  methods: {
    getMessage() { 
      //变更创建
      let handlerStepInfo = this.processTaskStepConfig.handlerStepInfo || null;
      if (handlerStepInfo) {
    ...
    changeValid() {
      //变更校验
      let validList = [];
      validList = this.$refs.changecreateData.changeValid();
