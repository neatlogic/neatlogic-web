...
export default {
  ...
  methods: {
      ...
      this.$createDialog({
        ...
        'on-ok': function(vnode) {
          ...
          if (!_this.disabledConfig.aborting) {
            _this.disabledConfig.aborting = true;
            this.$api.process.processtask.cancelTaskDetail(data).then(res => {
              _this.disabledConfig.aborting = false;
              if (res.Status == 'OK') {
                _this.toTask(_this.processTaskId);
              }
            }).catch(error => {
              _this.disabledConfig.aborting = false;
            });
    ...
    recoverTask() {
      //恢复
      if (!this.disabledConfig.recovertask) {
        this.disabledConfig.recovertask = true;
        let data = {
          processTaskId: this.processTaskId
        };
        this.$api.process.processtask.recoverTaskDetail(data).then(res => {
          this.disabledConfig.recovertask = false;
          if (res.Status == 'OK') {
    ...
    validItemClick(selector) {
      ...
      this.$refs.TaskCenterDetail.TabsVal = 'step';
    ...
    pauseStep() {
      //暂停工单步骤
      if (!this.disabledConfig.pausing) {
        this.disabledConfig.pausing = true;
        let data = {
          processTaskId: this.processTaskId,
          processTaskStepId: this.processTaskStepId
        };
        this.$api.process.processtask.pauseTaskStep(data).then(res => {
          this.disabledConfig.pausing = false;
          if (res.Status == 'OK') {
            this.toTask(this.processTaskId, this.processTaskStepId);
          }
        }).catch(error => {
    ...
    recoverStep() {
      //恢复工单步骤
      if (!this.disabledConfig.recoverstep) {
        this.disabledConfig.recoverstep = true;
        let data = {
          processTaskId: this.processTaskId,
          processTaskStepId: this.processTaskStepId
        };
        this.$api.process.processtask.recoverTaskStep(data).then(res => {
          this.disabledConfig.recoverstep = false;
          if (res.Status == 'OK') {
            this.toTask(this.processTaskId, this.processTaskStepId);
          }
        }).catch(error => {
    ...
    async taskValid() {
      ...
      if (this.handler == 'event') {
        let eventValidList = this.$refs.eventConfig.eventValid();
