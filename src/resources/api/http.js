...
const preLang = (text, value) => {
  ...
  let text1 = Vue.prototype.i18n.t(text, value);
...
const errorHandle = res => {
  ...
  switch (status) {
    case 401:
      ...
      break;
    case 403:
      tip(Vue.prototype.i18n.t('message.content.sessionexpired'), function() {
        Vue.prototype.$utils.removeCookie('neatlogic_authorization');
        toLogin();
      });
      break;
    case 404:
      console.log(Vue.prototype.i18n.t('message.content.urlnotfound', { target: res.config.url }));
      break;
    case 500:
      //已知的接口问题
      tip('服务器错误');
    ...
    case 523:
      ...
      Vue.prototype.$tsrouter.replace({
        path: '/404',
        query: {
          des: res.data && res.data.Message ? res.data.Message : Vue.prototype.i18n.t('message.content.noauth')
        }
      });
      break;
    case 524:
    ...
    default:
      try {
        console.log(Vue.prototype.i18n.t('message.content.urlnotfound', { target: res.config.url }) + '，原因：' + (res.data.Message ? res.data.Message : res.data));
      } catch (e) {
        throw e;
      }
  }
