...
export default class Gantt {
  ...
  setup_wrapper(element) {
    ...
    this.popup_wrapper.classList.add('ivu-poptip-popper');
    this.popup_wrapper.setAttribute('x-placement', 'right-start');
  ...
  setup_tasks(tasks) {
    ...
    this.tasks = tasks.map((task, i) => {
      ...
      //如果开始时间大于结束时间，修正结束时间=开始时间
      if (task._start && task._end && task._start.getTime() > task._end.getTime()) {
  ...
  setup_layers() {
    ...
    const headerLayers = ['header', 'date'];
    for (let layer of headerLayers) {
      this.layers[layer] = createSVG('g', {
        class: layer,
  ...
  make_grid_highlights() {
    ...
    if (this.view_is(VIEW_MODE.DAY)) {
      ...
      const height = (this.options.bar_height + this.options.padding) * this.tasks.length + this.options.padding / 2;
