  ...
  <div>
    <TsContain :enableCollapse="true" :siderWidth="260">
      <template v-slot:navigation>
        <span class="ts-angle-left text-action" @click="$back('/form-overview')">{{ $getFromPage('表单管理') }}</span>
      </template>
      ...
      <template v-slot:topRight>
        <div class="action-group">
          <div class="action-item">
            ...
            >
              <span class="tsfont-xitongpeizhi" @click="formValid()">校验</span>
              <div slot="content">
          ...
          <div class="action-item text-action tsfont-lightning" @click="openReactionDialog()">行联动</div>
          <div class="action-item text-action tsfont-width" @click="editFormWidth()">表单宽度</div>
          <div class="action-item text-action tsfont-scene" @click="openScene()">场景</div>
          <div class="action-item text-action tsfont-circulation-s" @click="previewForm()">预览</div>
          <div class="action-item">
            <Dropdown trigger="click">
              ...
              <DropdownMenu slot="list" class="dropdown">
                <DropdownItem v-if="currentVersion.uuid && referenceCount > 0" @click.native.stop="quoteList(1)">
                  <div class="ts-catalogue referenceCount">引用列表[{{ referenceCount }}]</div>
                </DropdownItem>
                <DropdownItem v-else-if="currentVersion.uuid">
                  <div class="action-item ts-catalogue referenceCount disable">引用列表</div>
                </DropdownItem>
                ...
                <DropdownItem>
                  <span class="action-item tsfont-trash-o" :class="activeVersionUuid == currentVersion.uuid && currentVersion.uuid ? 'disable' : ''" @click="delVersionModal(currentVersion.uuid, currentVersion.version)">删除</span>
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
          </div>
          <div class="action-item">
            <Button type="primary" ghost @click="saveForm('saveother')">另存为新版本</Button>
          </div>
  ...
  methods: {
    ...
    removeCustomItem(customItem) {
      this.$createDialog({
        ...
        content: '确定删自定义组件“' + customItem.label + '”？',
        btnType: 'error',
        'on-ok': vnode => {
    ...
    saveForm(type) {
      ...
      if (this.valid()) {
        ...
        !formConfig.name && (formConfig.name = '默认场景');
    ...
    changeVersion(uuid, text) {
      ...
      if (this.compareData(this.initFormConfig, formConfig)) {
        ...
        this.$createDialog({
          title: '提示',
          content: '当前表单有修改，是否确认在版本切换之前保存当前表单？',
          cancelText: '取消',
          okText: '保存',
          'on-ok': vnode => {
            vnode.isShow = false;
    ...
    async handleSaveForm(uuid) {
      ...
      if (isTip) {
        this.$createDialog({
          title: '提示',
          content: '当前表单已被未完成状态的工单引用，修改表单可能导致工单流转时无法正常输入表单信息，是否继续？',
          'on-ok': vnode => {
            if (uuid) {
    ...
    delVersionModal(uuid, version) {
      ...
      this.$createDialog({
        title: this.$t('dialog.title.deleteconfirm'),
        content: `确定要删除 <b>${uuid == this.currentVersion.uuid ? '当前' : version}版本</b> 的表单`,
        btnType: 'error',
        'on-ok': vnode => {
    ...
    beforeUpload(e) {
      ...
      if (this.$utils.isEmpty(this.$refs.sheet.validConfig()) && this.$refs.formName.valid()) {
        ...
        return new Promise(resolve => {
          this.$createDialog({
            title: '提示',
            content: '当前表单的版本校验不通过，是否确认导入时放弃未保存的修改？',
            cancelText: '取消',
            okText: '放弃',
            'on-ok': vnode => {
              vnode.isShow = false;
    ...
    exportFile() {
      ...
      if (this.$utils.isEmpty(this.$refs.sheet.validConfig()) && this.$refs.formName.valid()) {
        ...
        this.$createDialog({
          title: '提示',
          content: '当前表单的版本校验不通过，是否确认导出时放弃未保存的修改？',
          cancelText: '取消',
          okText: '放弃',
          'on-ok': vnode => {
            vnode.isShow = false;
    ...
    formValid() {
      if (this.valid()) {
        this.$Notice.success({title: '校验成功' });
      }
    },
    valid() { //整个表单校验
      this.errorData = {};
      ...
      if (!isValid || this.nameMsg) {
        this.$set(this.errorData, 'form', [{
          field: 'name',
          error: !isValid ? '表单名称：不能为空，仅支持汉字、字母、数字和下划线' : this.nameMsg
        }]);
        this.$nextTick(() => {
