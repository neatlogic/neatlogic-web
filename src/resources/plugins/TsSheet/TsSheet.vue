        ...
        <span class="action-item">
          <Dropdown :transfer="true" placement="bottom-start" trigger="click">
            ...
            <DropdownMenu slot="list">
              <DropdownItem @click.native="setStyle({ 'font-size': '12px' })"><span style="font-size:12px">小</span></DropdownItem>
              <DropdownItem @click.native="setStyle({ 'font-size': '14px' })"><span style="font-size:14px">中</span></DropdownItem>
              <DropdownItem @click.native="setStyle({ 'font-size': '16px' })"><span style="font-size:16px">大</span></DropdownItem>
              <DropdownItem @click.native="setStyle({ 'font-size': '18px' })"><span style="font-size:18px">最大</span></DropdownItem>
            </DropdownMenu>
          </Dropdown>
        </span>
    ...
    >
        ...
        <tbody>
          ...
          >
            ...
            <td
              ...
              @mouseenter="multipleSelectCell(cell)"
              @mousedown="
                event => {
                  if (mode === 'edit') {
                    clearSelectedRowCol();
            ...
            >
              ...
              <div v-if="!cell._isHandler && cell.border">
                <div style="border-top-width:1px;border-top-style:solid;position:absolute;top:0px;left:0px;height:1px;width:100%" :class="cell.border"></div>
                <div style="border-bottom-width:1px;border-bottom-style:solid;position:absolute;bottom:0px;left:0px;height:1px;width:100%" :class="cell.border"></div>
                <div style="border-left-width:1px;border-left-style:solid;position:absolute;top:0px;left:0px;height:100%;width:1px" :class="cell.border"></div>
                <div style="border-right-width:1px;border-right-style:solid;position:absolute;top:0px;right:0px;height:100%;width:1px" :class="cell.border"></div>
              </div>
            </td>
          </tr>
        </tbody>
      ...
      <div v-if="isContextMenuShow && mode === 'edit'" :style="{ top: contextMenuPosition.top + 'px', left: contextMenuPosition.left + 'px' }" class="tssheet-context-menu padding-sm">
        <Dropdown trigger="custom" :visible="isContextMenuShow">
          <DropdownMenu slot="list">
            <DropdownItem v-if="canMerge" class="cursor" @click.native="mergeCell">合并</DropdownItem>
            <DropdownItem v-if="canSplit" class="cursor" @click.native="splitCell">取消合并</DropdownItem>
            <DropdownItem
              v-if="canClear"
            ...
            >清除格式</DropdownItem>
            <DropdownItem :divided="canMerge || canSplit || canClear || canClearStyle" class="cursor" @click.native="insertRow('prepend')">向上插一行</DropdownItem>
            <DropdownItem class="cursor" @click.native="insertRow('append')">向下插一行</DropdownItem>
            <DropdownItem class="cursor" @click.native="insertColumn('prepend')">向左插一列</DropdownItem>
            <DropdownItem class="cursor" @click.native="insertColumn('append')">向右插一列</DropdownItem>
            <DropdownItem class="cursor" divided @click.native="deleteRow()">删除当前行</DropdownItem>
            <DropdownItem class="cursor" @click.native="deleteColumn()">删除当前列</DropdownItem>
            <DropdownItem
              v-if="hasCopy"
              class="cursor"
              divided
            ...
            >复制</DropdownItem>
            <DropdownItem v-if="hasCopy" class="cursor" @click.native="cutCell()">剪切</DropdownItem>
            <DropdownItem
              v-if="hasPaste"
              :divided="!hasCopy"
              class="cursor"
...
export default {
    ...
    pasteCell() {
      if (this.copyedCell && this.handlerCell) {
        if (this.handlerCell.component || this.handlerCell.content) {
          this.$createDialog({
            title: '警告',
            content: '当前单元格不为空，确认使用复制内容替换当前内容？',
            'on-ok': vnode => {
    ...
    mergeCell() {
      if (this.selectedCell.length > 1) {
        ...
        if (componentList.length > 0) {
          this.$createDialog({
            title: '提示',
            content: `合并单元格会清空以下组件：<br>${componentList.join('<br>')}`,
            'on-ok': vnode => {
              ok();
              vnode.isShow = false;
            }
  ...
  computed: {
    ...
    shownLefterList() {
      ...
      if (this.mode !== 'edit') {
        //只读模式，从下面消除所有没有任何设置的单元格
        for (let i = lefterList.length - 1; i >= 0; i--) {
          if (!this.config.tableList.find(cell => cell.row === i && (cell.component || cell.content || cell.style || cell.class || cell.border || this.checkCellIsInSpan(cell)))) {
