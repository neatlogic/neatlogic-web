        ...
        <Tabs v-if="reactionLocal">
          ...
          >
            <div v-for="(rect, index) in r" :key="index">
              <Divider plain orientation="center"><span class="tsfont-minus text-href" @click="removeReaction(r,index)">#规则{{ index+1 }}</span></Divider>
              <ConditionGroup
                :ref="'condition_' + key"
                :value="rect"
                ...
                "
              ></ConditionGroup>
              <div class="mt-sm mb-sm text-grey">影响行</div>
              <TsFormSelect
                :dataList="rowList"
                :value="rect.rows"
                multiple
            ...
            <Divider plain orientation="center"><span class="tsfont-plus text-href" @click="addReaction(r)">添加规则</span></Divider>
          </TabPane>
        </Tabs>
      </template>
    </TsDialog>
...
export default {
  ...
  data() {
    return {
      ...
      reactionName: { hiderow: '隐藏行', displayrow: '显示行' },
      dialogConfig: {
        title: '联动设置',
        width: 'medium',
        type: 'modal',
    ...
    rowList() {
      ...
      if (this.rowcount) {
        for (let i = 0; i < this.rowcount; i++) {
          dataList.push({ value: i, text: '第' + (i + 1) + '行' });
        }
      }
      return dataList;
    }
