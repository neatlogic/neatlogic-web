  ...
  <div>
    <TsContain>
      <template v-slot:navigation>
        <span class="ts-angle-left text-action" @click="gotoFormEdit()">默认场景</span>
      </template>
      ...
      <template v-slot:topRight>
        <div class="action-group">
          ...
          <div class="action-item text-action tsfont-circulation-s" @click="openPreview()">预览</div>
          <div class="action-item last">
            <Button type="primary" @click.stop="saveForm()">保存</Button>
          </div>
        </div>
      </template>
      <template v-slot:sider>
        <div class="formitem-container pr-sm">
          <Alert show-icon>
            <div>自定义场景仅支持从[默认场景]的已有组件中，选择组件</div>
          </Alert>
          <div>
            <TsRow :gutter="16">
              <Col
    ...
    valid() { //整个表单校验
      ...
      if (!this.sceneName) {
        this.$set(this.errorData, 'scene', [{
          field: 'name',
          error: '场景名称：不能为空，仅支持汉字、字母、数字和下划线'
        }]);
        this.$nextTick(() => {
        ...
        if (findName || this.sceneName === '默认场景') {
          this.$set(this.errorData, 'scene', [{
            field: 'name',
            error: '场景名称不能重复'
          }]);
