        ...
        <tbody v-if="optionList.length > 0">
          <tr v-for="(option, index) in optionList" :key="index">
            ...
            <td v-for="(type, tindex) in typeList" :key="tindex">
              ...
              <div v-else>
                <span v-if="isChecked(type, option)" :class="{ 'tsfont-circle-o': !config.isMultiple, 'tsfont-check-square': !!config.isMultiple }" class="text-success"></span>
  ...
  methods: {
    validConfig() {
      ...
      if (this.typeList.length === 0) {
        errorList.push({ field: 'typeList', error: '请至少添加一个类别' });
      }
      if (this.optionList.length === 0) {
        errorList.push({ field: 'optionList', error: '请至少添加一个选项' });
      }
      return errorList;
    },
    changeValue(val, type, option) {
