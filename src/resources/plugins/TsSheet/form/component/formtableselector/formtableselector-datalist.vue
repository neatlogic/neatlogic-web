  ...
  methods: {
    ...
    searchMatrixData(currentPage) {
      ...
      this.$api.framework.matrix.getNewMatrixDataForTable(this.searchParam).then(res => {
        //编辑模式下只显示表头
        if (this.mode !== 'read') {
          this.$set(this.matrixData, 'theadList', res.Return.theadList);
        } else {
          this.matrixData = res.Return;
          this.matrixData.theadList = [{key: 'selection'}];
          this.config.dataConfig.forEach(d => {
            if (d.isPC) {
              this.matrixData.theadList.push({
                key: d.uuid,
                title: d.label,
                type: d.config && d.config.urlAttributeValue ? 'linktext' : '',
                textValue: d.config && d.config.urlAttributeValue ? d.config.urlAttributeValue : '',
                isRequired: !!(d.config && d.config.isRequired)
              });
            }
          });
      ...
      handler: function(val) {
        if (!this.formItem.isEditing) {
          ...
          if (this.config && this.config.matrixUuid && this.config.matrixUuid != this.searchParam.matrixUuid) {
            ...
            needReload = true;
          } else if (this.config && !this.config.matrixUuid) {
            this.$set(this.matrixData, 'theadList', []);
          }
          if (needReload) {
