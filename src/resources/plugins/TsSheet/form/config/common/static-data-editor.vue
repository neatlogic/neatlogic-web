...
<template>
  <div>
    <div class="item-grid">
      <div class="text-grey">值</div>
      <div class="text-grey">名称</div>
      <div v-if="needBatchEdit" style="text-align:right"><span :class="disabled?'text-disabled':'text-href'" @click="openBatchEdit()">批量编辑</span></div>
    </div>
    <draggable
      ...
      <div v-for="(item, index) in dataListLocal" :key="index" class="item-grid">
        ...
        <div style="text-align:right">
          <span
            ...
            class="tsfont-plus-o"
            :class="disabled?'text-disabled':'text-text-action'"
            title="新增"
            @click="addData(index)"
          ></span>
          <span
    ...
    <div v-else><span v-if="!disabled" class="tsfont-plus text-href" @click="addData()">添加数据</span></div>
    <TsDialog
      v-if="isBatchEdit"
      :isShow="true"
      type="modal"
    ...
    >
      <template v-slot>
        <div class="text-grey">
          <p>1、每一行代表一个选项，且每一行的值不可以重复。</p>
          <p>2、值和名称用","分隔，可以只有值，名称会自动补充。</p>
          <p>3、使用符号"-"作为前缀代表层级关系，有多少个"-"代表多少级，最多不超过5级。</p>
          <p>4、数组长度不可大于500。</p>
        </div>
        <TsFormInput
