<template>
  <div>
    ...
    <TsFormItem label="样式" labelPosition="top">
      <div class="padding-sm bg-op radius-sm">
        <TsFormRadio
    ...
    <!-- 校验 -->
    <TsFormItem label="校验" labelPosition="top">
      <div class="padding-sm radius-sm" :class="validClass('validValueList')">
        ...
        <div v-if="checkValidType('custom')">
          ...
          <span v-if="!disabled" class="tsfont-plus text-href" @click="addFilter">规则</span>
        </div>
      </div>
    </TsFormItem>
...
export default {
  ...
  data() {
    return {
      formatList: [
        { value: 'yyyy-MM-dd HH:mm', text: '年-月-日 时:分' },
        { value: 'yyyy-MM-dd', text: '年-月-日' },
        { value: 'yyyy-MM', text: '年-月' },
        { value: 'yyyy', text: '年' }
      ],
      filterDataList: [
        { text: '大于', value: 'later' },
