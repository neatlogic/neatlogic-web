<template>
  <div>
    <TsFormItem label="选择方式" labelPosition="left" contentAlign="right">
    ...
    >
      <div class="radius-sm padding-md" :class="validClass('matrixUuid')">
        ...
        <div v-if="!disabled" class="matrix-btn">
          ...
          ></i>
          <i class="tsfont-rotate-right pl-xs text-tip-active" title="刷新" @click="refreshMatrixConfig(config.matrixUuid)"></i>
        </div>
      </div>
    </TsFormItem>
    <TsFormItem
      v-if="config.dataSource === 'matrix' && config.matrixUuid"
      tooltip="勾选PC端/移动端需要展示或搜索的属性，搜索和显示列的显示互不影响"
      label="矩阵属性"
      labelPosition="top"
    >
      <div class="padding-md radius-md bg-block">
        <div class="tstable-container tstable-normal radius-lg">
          <table class="tstable-body">
            <thead>
              <tr>
                ...
                <th><i class="tsfont-search" title="搜索条件"></i></th>
                <th></th>
              </tr>
            </thead>
            <draggable
          ...
          <div class="margin-sm" style="text-align:center">
            <Button style="width:100%" :disabled="disabled" @click="addExtraProperty()"><span class="tsfont-plus">扩展属性</span></Button>
          </div>
        </div>
      </div>
    </TsFormItem>
    ...
    <TsFormItem v-if="config.matrixUuid && selectMatrixConfig" labelPosition="top" label="过滤条件">
...
export default {
  ...
  data() {
    return {
      modeList: [
        { value: 'dialog', text: '弹窗选择' },
        { value: 'normal', text: '直接选择' }
      ],
      dataSourceList: [
        { value: 'matrix', text: '矩阵' },
        { value: 'integration', text: '集成' }
      ],
