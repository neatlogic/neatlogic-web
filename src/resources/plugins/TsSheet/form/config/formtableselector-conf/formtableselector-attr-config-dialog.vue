      ...
      <div v-if="propertyLocal.isExtra">
        <TsForm ref="formitem_base" v-model="propertyLocal" :item-list="formConfig">
          ...
          <template v-else-if="['formselect', 'formradio', 'formcheckbox'].includes(propertyLocal.handler)" v-slot:config>
            ...
            <TsFormItem v-if="propertyLocal.config.dataSource === 'matrix'" :label="$t('page.matrix')" required>
              <div>
                <Row>
                  <Col span="22">
                    <TsFormSelect
                  ...
                  <Col span="2">
                    <QuickOperation
                      :config="{
                        ...
                        name: propertyLocal.matrixName || selectMatrixConfig?.name,
                        type: propertyLocal.matrixType || selectMatrixConfig?.type,
                      }"
                      @refresh="()=> {
