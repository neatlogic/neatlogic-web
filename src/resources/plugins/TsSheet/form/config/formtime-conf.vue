      ...
      <div class="padding-sm radius-sm" :class="validClass('validValueList')">
        ...
        <div v-if="checkValidType('custom')">
          ...
          >
            ...
            <i class="tsfont-close-o valid-remove text-tip-active" title="删除" @click="removeFilter(vindex)"></i>
          </TsRow>
          <span class="tsfont-plus text-href" @click="addFilter">规则</span>
        </div>
      </div>
    </TsFormItem>
...
export default {
  ...
  data() {
    return {
      formatList: [
        { value: 'HH:mm:ss', text: '时:分:秒' },
        { value: 'HH:mm', text: '时:分' }
      ],
      filterDataList: [
        { text: '晚于', value: 'later' },
