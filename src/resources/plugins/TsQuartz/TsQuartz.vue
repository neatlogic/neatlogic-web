<template>
  <div>
    <div
      class="jquery-quartz-container"
      :class="{ 'quartz-show': showType != 'edit' }"
    >
      <div v-for="(controller, index) in controllerList" :key="index" class="cron-controller">
        ...
        >
          <div class="jquery-quartz-controller-text cursor ml-xs" @click.stop="showPoptip(controller.datasource)">
            <span v-if="controller.datasource === 'controller'">{{ Setting.dict[tmpCronType || (!tmpCronType && cronType)] }}</span>
            <span v-else>{{ getCronDataValueName(controller) }}</span>
          </div>
