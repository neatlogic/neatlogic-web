...
export default {
  ...
  props: {
    ...
    idListName: {//实时搜索，进行回显时，调用接口的传参进行精确匹配
      type: String,
    ...
    filterName: { //搜索过滤关键词,如果值为空默认为[textName,valueName],如果值不为空则以filterName为准,keyword query匹配到item[filterName]时，该选项会在搜索时显示
      type: [String, Array]
    },
    showName: String,
    validateList: {
    ...
    placement: {//dropdown显示的方位，因为计算位置的原因导致有些情况需要计算
      default: 'bottom-start'
    },
    needCallback: {//是否搜索需要回调，可以定义回调事件,配合searchCallback和:needCallback.sync使用
      type: Boolean,
      default: false
    },
    firstLi: {//是否添加第一个
    ...
    firstSelect: {//value为空，必填且下拉列表只有一个时是否需要选中第一个
      type: Boolean,
      default: true
    },
    nowrapHead: {//当出现多选的时候，不换行显示，如果数据过多时则显示不全
      type: Boolean,
      default: false
    },
    isSquare: {
  ...
  data: function() {
    ...
    return {
      ...
      moreSearchTip: {
        [_this.showName ? _this.showName : _this.textName]: '更多请搜索',
  ...
  methods: {
    ...
    initReadolyTitle() { //主要 用来显示tooltip,当只读的时候显示的内容不完善时，通过tooltip来提示
      if (!this.$el) {
        return;
      }
    ...
    async getDataByAjax(params, url, cancel) { //调用接口的统一处理
    ...
    setDefaultValue() { //默认选中第一个的判断 ,1、defaultValueIsFirst 2、必填且nodeList长度为1
    ...
    creatNewItem(keyword) { //通过enter创建一个新的数据选项
    ...
    initDataListByUrl(isSearch) {
      ...
      if (this.url && !isSearch) { //url
      ...
      } else if (_this.dynamicUrl) { //dynamicUrl
        ...
        if (!this.$utils.isEmpty(this.value)) {
          ...
          this.currentPage = 1;
          this.reachPage = 0;
          if (this.readonly || this.disabled) { //初始化：只读且没有选中时，不需要调接口
            return;
          }
    ...
    initValueByNodeList() {
      ...
      if (this.mode == 'normal') {
        ...
        this.nodeList.forEach((pitem) => {
          if (pitem[this.childrenName] && pitem[this.childrenName].length > 0) {
            pitem[this.childrenName].forEach(item => { ArrIndexOf(ary, item[this.valueName]) >= 0 && selectedList.push(item); });
          }
        });
      }
      ...
      if (selectedList.length > 1) {
        this.currentValue instanceof Array && this.currentValue.forEach(item => {
      ...
      if (!this.multiple) { //单选时需要给searchKeyword赋值
    ...
    deleteSeleted(ind, value, list) {
      if (!this.multiple) {
        ...
        this.$refs.input && this.$refs.input.focus();
        this.onChangeValue();
        if (this.selectedList.length === 0) { // 如果没有数据的时候，就请求一次清空的方法，不然会有bug
          this.clearValue();
        }
      }
    },
    ...
    dynamicSearch(query, isFirst) {
      ...
      this.getDataByAjax(params, this.dynamicUrl, 'cancelAxios').then(res => {
    ...
    toggleSelect(item) {
      ...
      } else if (this.isRequired && !this.multiple) { //取消选中  如果必填且单选，则不能取消选中
        return;
      } else { //取消选中
    ...
    onClickOutside(event) {
      //点击外部，dropdown消失
      if (this.isVisible || this.firstOutside === false) { //this.firstOutside为false 代表select处于聚焦的转态，在点击外面的时候需要给searchKeyWord赋值
  ...
  watch: {
    ...
    selectedList: {//选中列表发生变化时需要触发更新label的方法,label类型为[string, array]
      handler() {
