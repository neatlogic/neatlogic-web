...
(function(global, factory) {
    ...
    if (d.el && d.linkEl) {
      ...
      if (value) {
        ...
        d.nameEl = d.el
          ...
          .attr('opacity', '1')
          .attr('font-size', '12px')
          .attr('filter', `url(#linktextbg${d.canvas.getUuid()})`);
        //反转字体
        if (pos.tx < pos.sx) {
    ...
    if (d.linkEl) {
      if (value) {
        if (!d.getArrowreverse()) {
          if (!d.markerEnd) {
            d.markerEnd = d.canvas.defs
              .append('marker')
              .classed('linkArrow', true)
              .attr('id', `arrowEnd${d.getUuid()}`)
          ...
          if (!d.markerEndReverse) {
            //反向箭头
            d.markerEndReverse = d.canvas.defs
              .append('marker')
              .classed('linkArrow', true)
              .attr('id', `arrowEndReverse${d.getUuid()}`)
              .attr('markerUnits', 'strokeWidth')
              .attr('markerWidth', '12')
              .attr('markerHeight', '14')
              .attr('viewBox', '0 0 12 12')
  ...
  PH['link.markerstart'] = function(d, value) {
    if (d.linkEl) {
      if (value) {
        if (!d.markerStart) {
          d.markerStart = d.canvas.defs
            .append('marker')
            .classed('linkArrow', true)
            .attr('id', `arrowStart${d.getUuid()}`)
