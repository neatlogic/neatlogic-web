...
;(function () {
    ...
    TsFlowChart.prototype = {
        init: function (container, opts) {
            var that = this;
            if (this.consoleLog(container, $t('message.targetnoexist', {target: 'container'}))) return false;
            // uuid arr
            this.uuidArr = [];
    ...
    TsFlowChart.prototype.extend({
        fromJson: function (json) {
            if (this.consoleLog(json, $t('message.invalidjson'))) return false;
            this.Json = json;
        ...
        valid: function () {
            ...
            nodes.each(function (d) {
                    ...
                    Object.keys(d.valid).forEach(function (k) {
                        if (d.behavior[k] === false || d.behavior[k] === true) {
                            ...
                            if (d.behavior[k] === false && d.valid[k] === true) {
                                if (k === 'needIn') {
                                    msg = $t('message.plugin.cannotlinein');
                                } else if (k === 'needOut') {
                                    msg = $t('message.plugin.cannotlineout');
                                }
                            } else if (d.behavior[k] === true && d.valid[k] === false) {
                                if (k === 'needIn') {
                                    msg = $t('message.plugin.nolinein');
                                } else if (k === 'needOut') {
                                    msg = $t('message.plugin.nolineout');
                                }
                            } else {
                ...
                if (d.data.type !== 'start' && d.data.type !== 'end') {
                    ...
                    if (d.behavior.needIn) {
                        ...
                        if (isStart === false) {
                            msg = $t('message.plugin.notalonebystart');
                        }
                    }
                    if (d.behavior.needOut) {
                        var isEnd = that.isConnectEnd(_this);
                        if (isEnd === false) {
                            msg = $t('message.plugin.notalonebyend');
                        }
                    }
                    if (msg) {
                        if (!that.include(d.tipData.error, msg)) d.tipData.error.push(msg);
            ...
            errorNode.forEach(function (d1) {
                d1.tipData.msg = `${$t('message.plugin.thisnode')}：\n` + d1.tipData.error.join('\n');
            });
