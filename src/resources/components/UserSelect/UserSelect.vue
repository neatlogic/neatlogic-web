              ...
              <ul v-if="group.dataList && group.dataList.length > 0">
                ...
                >
                  ...
                  <Tooltip
                    ...
                    transfer
                  >
                    <slot name="option" :row="user" :group="group"><div class="overflow team-text">{{ user.text }}</div> </slot>
...
export default {
  ...
  watch: {
    value: {
      handler: function(val) {
        ...
        if (this.selectedList.length) {
          let label = this.multiple ? this.selectedList.map(s => { return s.text; }) : this.selectedList[0].text;
    ...
    selectedList: {//选中列表发生变化时需要触发更新label的方法,label类型为[string, array]
      handler() {
        let label = this.multiple ? this.selectedList.map(s => s.text) 
          : this.selectedList.length > 0 ? this.selectedList[0].text : '';
