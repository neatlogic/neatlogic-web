...
1. `ReferenceSelect` 重新打开下拉时会重置分页状态和滚动加载提示，但不会改变原有接口参数、返回结构和展示语义。
2. 滚动加载增加了定时器清理和请求序号保护，旧请求不会再覆盖新列表，最后一页分支也会正常结束 Promise。
3. `ReferenceList` 同步增加了请求序号保护，快速切换 `id`、`uuid` 或重复触发加载时，组件只保留最后一次有效响应。
