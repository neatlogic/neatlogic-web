...
export default {
  ...
  props: {
    ...
    searchMode: {
      type: String, // 搜索模式可选：自动搜索(automatic)，手动搜索(manual)
      default: 'automatic' // 默认自动搜索
    }
  },
  data() {
    return {
      isCloseAutomaticSearch: false, // 是否关闭自动搜索
      isVisible: false,
      keyword: '', //输入框的值
      searchConfig: {
        size: 'small',
  ...
  methods: {
    ...
    handleToggleOpen() {
      ...
      if (this.isVisible && this.searchMode == 'manual') {
        this.isCloseAutomaticSearch = true; // 关闭自动搜索
      }
    },
    initWidth() {
    ...
    removeItem(key) {
      ...
      if (this.searchValue && Object.keys(this.searchValue).length && Object.keys(this.searchValue).includes(key)) {
        this.$nextTick(() => {
          ...
          if (this.searchMode == 'manual') { // 手动搜索模式，删除后，需要触发一次change
    ...
    clearSearch() {
      ...
      if (this.searchMode == 'manual') {
        this.$emit('change', {}, this.searchMode);// 手动搜索，删除全部数据后，触发change
      }
    },
    handleSearch() {
