...
try {
  ...
  process.env.VUE_APP_CUSTOMPAGES = projectName.home;  //当前模块是否是苏州银行的
  pageTitle = projectName.title;
  process.env.VUE_APP_LOGINTITLE = projectName.loginTitle; // 登陆页标题
  process.env.VUE_APP_TABLESTRYLE = projectName.tableStyle; //table显示的间隔是边框，之所以在这里定义table的显示样式，因为苏州模块需要所有的table都是颜色间隔显示，然而产品的显示样式为边框间隔
} catch (e) {
  baseImg = './public/resource';
